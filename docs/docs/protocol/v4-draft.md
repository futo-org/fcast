...
maximum packet size. Because `Total Parts` is a `U8`, the requester must keep each
`ResourceReadHead` range small enough to be delivered in at most 256 parts.
