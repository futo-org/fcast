...
/// A bitmap cue is a RUN of buffers, not one. Subpicture formats spread a
/// display set over several packets that share a presentation time, and only
/// the decoder's reassembly of them means anything. So the schedule puts every
/// chunk at the cue's start, in order, byte-for-byte as given, and gives a
/// duration only to a lone chunk, because claiming each fragment displays for
/// the whole cue would be false.
#[test]
fn a_bitmap_cues_chunks_share_one_pts() {
    // `pgs_caps()` builds a `gst::Caps` before the harness is constructed, and
    // that needs the library up.
    init();
    let cues = vec![
        CueSpec::packets(
            gst::ClockTime::from_mseconds(50),
            gst::ClockTime::from_mseconds(150),
            vec![vec![0x20, 0x00], vec![0x0f, 0x10, 0x00], vec![0xff]],
        ),
        CueSpec::packets(
            gst::ClockTime::from_mseconds(250),
            gst::ClockTime::from_mseconds(350),
    ...
    let spec = MediaSpec::new(11).with_stream(
        StreamSpec::text("text_0", cues)
            // The widened pad template has to admit these caps, or the pad
            // refuses them before a single buffer is scheduled.
            .with_caps(caps::pgs_caps())
            .with_duration(TEN_FRAMES),
    );
    let (harness, _scenario) = Harness::new("schedpackets", spec);
    harness.play();
    harness.wait_for_eos();
    ...
    let at = |ms: u64| Some(gst::ClockTime::from_mseconds(ms));
    assert_eq!(log.pts(), vec![at(50), at(50), at(50), at(250)]);
    assert_eq!(
        log.buffers
            .iter()
        ...
        "the chunks must arrive verbatim and in order"
    );
    assert_eq!(
        log.buffers
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec![None, None, None, at(100)],
        "only a lone chunk carries the cue's duration"
    );
    // The gaps are the cue's, not the fragments'. A run of same-pts buffers
    // occupies one span of the timeline between them.
    assert_eq!(
        log.gaps,
        vec![
            (gst::ClockTime::ZERO, at(50)),
            (gst::ClockTime::from_mseconds(150), at(100)),
            (gst::ClockTime::from_mseconds(350), at(50)),
        ]
    );
}
...
fn flushing_offset_seek_restarts_at_the_target() {
    ...
    // Audio has no keyframes to walk back to, so packet 11 spans the target.
    assert_eq!(audio.pts()[2], Some(packet * 11u64), "audio restart pts");
...
fn offset_seek_past_the_end_goes_straight_to_eos() {
    ...
    assert!(
        harness.src.send_event(seek),
        "the past-the-end seek was refused"
    );
    gate.release();
    harness.wait_for_eos();
...
/// The fault must be byte-identical to a real source giving up. Consumers read
/// "reason not-linked" / "reason flushing" out of the debug string and recover
/// in place, treating anything else as a genuine failure. Asserting less would
/// let the text drift and the fault would come to mean the opposite.
#[test]
