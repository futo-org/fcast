...
//! scenario file a lie, so the assertions are on the OUTPUT, never on the
//! parsed `MediaSpec`.
...
fn the_generated_media_matches_the_document() {
    ...
    let cues = [("FIRST", 100u64, 250u64), ("SECOND", 400u64, 500u64)];
...
/// `duration_ms` is what the source reports upstream, not just what it
/// schedules.
#[test]
fn the_reported_duration_matches_the_document() {
    ...
    let gst::GenericFormattedValue::Time(duration) = query.result() else {
        panic!(
            "the duration came back in a format other than time: {:?}",
            query.result()
        );
    };
    assert_eq!(
...
/// A document must not describe media that does not exist. A stream whose
/// schedule holds no whole buffer would be an immediate EOS while the file
/// still claims the stream, so parsing refuses it.
#[test]
