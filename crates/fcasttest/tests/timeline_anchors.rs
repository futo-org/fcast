...
//! bounded wait on something the pipeline actually did [...] No wall-clock
//! sleep is ever a correctness wait". These tests hold the module to that,
//! including the case a seek creates: the SAME gate parked on a second time.
...
/// The timeline exposes the same anchor, so a test never has to poll
/// `arrivals()` in a sleep loop to find the second park.
#[test]
...
/// An anchor that is never reached fails inside its bound instead of hanging
/// the suite, and says what it was waiting for.
#[test]
