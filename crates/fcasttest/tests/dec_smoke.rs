...
impl Harness {
    /// `appsrc -> decoder -> appsink(sync=false)`, already PLAYING. The
    /// stream-id appsrc invents is rewritten to `key`/`suffix` so the
    /// decoder resolves its knobs from the registry.
    ...
    /// Latency the decoder adds on top of its (non-live, zero latency)
    /// upstream.
...
/// No registered scenario at all. The knobs default to off and the element
/// still decodes, which is what makes it usable outside a scenario.
#[test]
...
fn video_needs_keyframe_after_flush() {
    ...
    // Re-armed by the flush, so only the keyframe and what follows it decode.
    harness.flush();
...
/// Jitter only widens the reported maximum, every frame still comes out. The
/// draw itself is seeded, see the prng unit tests.
#[test]
