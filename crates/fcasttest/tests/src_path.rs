...
//! parsed pad when a decoder for the caps exists. With none it marks the chain
//! a deadend, posts missing-plugin and exposes nothing.
...
/// keep their default async handling, so no branch can drain before the
/// pipeline runs and every stream is represented when EOS is decided.
