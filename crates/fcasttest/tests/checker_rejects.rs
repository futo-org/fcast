...
//! it legitimately can. `check_all_named` has to reject the first set and accept
//! the second. Both directions matter: a checker that rejects everything is as
...
/// legal log, which is the point: the checker has to notice the mutation.
...
/// Sequences a correct pipeline really does produce. A checker that flags any of
/// these would make every test built on it flaky.
fn legal() -> Vec<(&'static str, Vec<RecordEntry>)> {
    ...
    // Tightening this to zero looks like a stronger checker and is really a flake:
    // an EOS past the flushing check and preempted before the event function
    // records itself produces exactly this. Two entries is what proves a flush was
    // ignored, see asserts::nothing_serialized_during_flush.
    let mut case = opened();
...
/// The sweep has to report EVERY offender, not stop at the first one, and it has
/// to name them: a violation nobody can attribute is a violation nobody fixes.
#[test]
...
/// reject it now.
///
/// This is the test that would have caught the original hole. `asserts::all` was
/// accepting five classes of illegal sequence, and nothing in the suite compared
/// what the rules covered against what a sink can actually observe.
#[test]
...
/// `first_buffer_is_discont` is not part of `all` (see its documentation), so it
/// gets its own mutation pair. `toml_scenarios.rs` is what runs it against real
/// media.
#[test]
