//! The fuzz driver replays crashes through `to_toml` dumps. That only holds if
//! the dumped document parses AND describes the same media, so this file feeds
//! it the shapes the builder can build but the document nearly cannot.
...
/// real media has (30000/1001, 24000/1001). The builder takes a
/// `gst::Fraction`, so a scenario can carry one, and the dumped document has to
/// carry it back.
#[test]
...
/// The everyday shape, so a regression in the awkward cases cannot be "fixed"
/// by breaking the normal one.
#[test]
...
/// A bitmap cue's payload is bytes, not text. The dump writes byte arrays and
/// the reader must return the same fragments in the same order, including the
/// empty packet and the zero byte that a naive string encoding loses.
#[test]
fn a_bitmap_cues_packets_survive_the_dump() {
    round_trip("rtpackets", |builder| {
        builder.video("video_0").text(
            "text_0",
            vec![
                CueSpec::packets(
                    gst::ClockTime::from_mseconds(100),
                    gst::ClockTime::from_mseconds(400),
                    vec![vec![0x20, 0x00, 0x0f, 0x10], vec![0x00, 0xff], vec![]],
                ),
                // One chunk: the shape that carries a duration.
                CueSpec::packets(
                    gst::ClockTime::from_mseconds(500),
                    gst::ClockTime::from_mseconds(800),
                    vec![vec![0x00]],
                ),
                // Text and bitmap cues in one stream, so the dumper cannot get
                // away with picking a payload kind per STREAM.
                CueSpec::new(
                    gst::ClockTime::from_mseconds(900),
                    gst::ClockTime::from_mseconds(950),
                    "CUE02",
                ),
            ],
        )
