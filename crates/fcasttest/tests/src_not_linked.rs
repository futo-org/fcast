...
//!
//! parsebin unlinks `typefind:src` from its parse pad while it plugs the parser.
//! fcasttest caps are sticky, so typefind classifies from the CAPS event alone and
//! forwards the first buffer straight into that window. Here the window is made
//! explicit: the src pad is left peerless for [`UNLINKED_WINDOW`] and then linked.
//! Nothing may be lost and nothing may be posted. A pad that is never linked still
//! has to fail, which is the other half of what ftestsrc's bound buys.
...
/// Wider than the parsebin plug window by an order of magnitude, and well inside
/// ftestsrc's retry bound.
...
/// Blocks until pad-added ran. It fires from inside the state change, so this is
/// satisfied by the time the transition returns.
...
/// A pad with no peer for the first 50 ms must cost nothing: every buffer still
/// arrives, in order, and the source posts nothing.
#[test]
...
/// The bound is a bound, not a licence: a pad nothing ever links to still posts the
/// stream error every demuxer posts for a fatal flow return, and it does so without
/// spinning forever.
#[test]
fn a_pad_that_is_never_linked_still_errors() {
    ...
    match wait_for_outcome(&pipeline, TIMEOUT) {
        Outcome::Error(err) => assert!(
            // basesrc's exact phrasing: fcastplaybin's recover-in-place
            // classification keys on this debug text (see
            // decisions::external_error_action).
