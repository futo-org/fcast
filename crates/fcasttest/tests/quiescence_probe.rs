...
//! call, so the exact strength of the guarantee decides whether those sweeps
//! are racing. These tests pin it in both directions: what it does catch, and
//! the shapes it provably does not.
...
/// invariant. Both are correct in isolation and together they are a silent
/// pass, so a caller has to assert that data arrived BEFORE it sweeps.
#[test]
