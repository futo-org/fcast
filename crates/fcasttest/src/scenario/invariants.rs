...
///
/// An empty log breaks no rule, and neither does an empty slice, so this
/// passing does NOT mean anything played. Assert that data arrived separately.
...
/// [`check_all`] with caller-supplied names. EVERY failure is reported, not
/// just the first. A wedge usually violates the same rule on several sinks.
...
/// [`check_all`] sees a quiescent log. False when something was still producing
/// at `bound`.
///
/// A settle heuristic and nothing stronger. It cannot tell "the stream ended"
/// from "the next buffer is more than `settle` away", so `settle` must exceed
/// the widest inter-arrival gap the media can produce, and nothing checks that
/// it does. An empty log satisfies it too. Anchor on the event that says the
/// work is done (EOS, a confirmed selection, a completed stop) and use this
/// only to let its tail land.
