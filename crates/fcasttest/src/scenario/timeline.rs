...
//! pipeline actually did: a push parking on a gate, buffers or an event
//! reaching a sink. No wall-clock sleep is ever a correctness wait, and no
...
/// Bound every anchor waits under. Generous, so a busy box does not flake and
/// a wedge does not hang the suite.
...
impl Timeline {
    ...
    /// action provably lands mid-push. Returns immediately once anything
    /// ever arrived.
    pub fn on_sync_point_arrival(&self, name: &str) -> Result<(), TimelineError> {
        self.on_sync_point_arrivals(name, 1)
    }

    /// Blocks until `count` pushes have parked on the named sync point in
    /// total.
    ///
    /// The counted form matters once a schedule restarts.
    /// [`on_sync_point_arrival`](Self::on_sync_point_arrival) is already
    /// satisfied by the FIRST park, so it returns instantly and the next
    /// action races the restarted push. It is also the anchor for "both
    /// streams are parked".
    ...
    /// Lets a parked push continue. Idempotent, and safe to call before
    /// anything has arrived (the gate stays open).
