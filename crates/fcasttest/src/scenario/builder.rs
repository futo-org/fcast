...
/// jitter draw (see [`stable_seed`]) and appears in every stream-id. Two runs of
/// the same test therefore describe byte-identical media.
...
impl ScenarioBuilder {
    ...
    /// Applied to EVERY stream at [`register`](Self::register) time, whatever the
    /// individual specs say. Scenarios are usually described as one clip.
    ...
    /// The media exactly as [`register`](Self::register) will store it, with the
    /// top-level overrides already applied to every stream. A front-end that
    /// wants to check a description before anything is registered has to look at
    /// this and not at the per-stream entries, which the overrides still win over.
    ...
    /// Registers the scenario, replacing any previous entry under the same key.
    /// Call before building the pipeline: registering discards stashed handles.
...
/// A registered scenario, as tests hold it. Cheap to clone (handles share the
/// registry entry), and there is nothing to clean up on drop: a push parked on a
/// sync point is unparked by the flush or teardown that ends the pipeline, so a
/// panicking test cannot leave a stuck streaming thread behind.
#[derive(Clone)]
...
impl ScenarioHandle {
    ...
    /// whose sink the test cannot reach (an autoplugged branch) still exposes
    /// one. The element is returned for tests that place it themselves.
