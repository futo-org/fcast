//! `ftestvdec` and `ftestadec`: knob-driven decoders. Owner: agent B.
//!
//! Both subclass the GStreamer decoder base classes, so flush, drain, segment and
//! latency handling stay on the field code paths. decodebin3 autoplugs these, which
//! rules out configuring them through properties, so the knobs are resolved from the
//! scenario registry on every STREAM_START. A stream-id with no registered scenario
//! resolves to all-off defaults, which keeps both elements usable standalone.
//!
...
mod knobs {
    ...
    pub struct State {
        knobs: DecoderKnobs,
        prng: Prng,
        /// Input frames handed to this decoder since the last STREAM_START.
        /// Monotonic across flushes, so `error_at_frame` indexes the input stream
        /// and not a post-seek run.
        index: u64,
    ...
    impl State {
        /// Rebinds to the stream that just started. Unknown stream-ids resolve to
        /// all-off defaults.
        ...
        /// reorder delay is deliberately not folded in, scenarios that care state
        /// it through the latency knob.
...
mod video {
    ...
    mod imp {
        ...
        /// Flat per-plane fill keyed on the frame index, enough for sinks to consume
        /// and for tests to tell frames apart.
...
mod audio {
    ...
    mod imp {
        ...
        impl FTestAdec {
            /// Pushes out every packet the reorder knob is holding back. One input
            /// frame is consumed per packet, which keeps the base class timestamp
            /// bookkeeping aligned.
            fn release_held(&self) -> Result<(), gst::FlowError> {
                let dec = self.obj();
                loop {
        ...
        impl AudioDecoderImpl for FTestAdec {
            ...
            fn set_format(&self, input_caps: &gst::Caps) -> Result<(), gst::LoggableError> {
                // Raw output is always stereo, the src template fixes it. Only the
                // rate follows the input caps.
                let rate = input_caps
                    .structure(0)
            ...
            fn handle_frame(
                ...
                let (index, delay) = match step {
                    ...
                    Step::Drop { index } => {
                        ...
                        // Consume the input frame without output, otherwise the base
                        // class would timestamp later packets from it.
        ...
        /// One output packet per input frame. The input duration decides its length,
        /// so output timestamps stay contiguous.
