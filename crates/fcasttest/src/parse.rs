//! `ftestparse`: near-identity parsers for the fcasttest caps. Owner: agent A.
//!
//! Exists so `urisourcebin parse-streams=true` builds the same parsebin chain it
//! builds in the field. The src caps add `parsed=true`, which is what lets
//! parsebin stop parsing and expose the stream once it sees a decoder next (the
//! decoders' sink templates omit the field, so they match either way).
//!
//! Registered above the decoders' PRIMARY, the way h264parse outranks the H.264
