...
//!
//! Autoplugged elements cannot be handed test state through properties, so every
//! element resolves what it needs here: ftestsrc by URI key, ftestdec by the key it
//! parses out of its sink-pad stream-id.
...
impl Scenario {
    ...
    /// Stashes an element-side handle (a sink recording, for example) so the test
    /// can reach it without fcasttest depending on the element's type here.
...
/// One-shot named gate. A stalled push blocks on it from the streaming thread and
/// the scenario runner releases it by name.
...
impl SyncPoint {
    ...
    /// Blocks until released or the timeout expires. Returns whether it was released.
    ...
    /// Blocks until at least one thread has reached the gate. Lets a test know a
    /// push is parked before it drives the next action.
    ///
    /// Arrivals are cumulative and never reset, so this is satisfied forever once
    /// anything has hit the gate. A schedule that is restarted (by a flushing
    /// seek, say) parks on the same gate again, and waiting for THAT needs
    /// [`wait_for_arrivals`](Self::wait_for_arrivals) with the count.
    pub fn wait_for_arrival(&self, timeout: Duration) -> bool {
        self.wait_for_arrivals(1, timeout)
    }

    /// Blocks until `count` threads have reached the gate in total. The counted
    /// form of [`wait_for_arrival`](Self::wait_for_arrival), so a test can anchor
    /// on the second park of a restarted schedule instead of polling
    /// [`arrivals`](Self::arrivals) in a sleep loop.
