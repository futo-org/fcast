//! `ftestsrc`: source element and `ftest://` URI handler.
//!
//! One sometimes src pad per [`crate::spec::StreamSpec`], each driven by its
//! own streaming task, so a scenario can stall, error or flush a single stream.
...
mod imp {
    ...
    use crate::{
        ...
        spec::{
            AUDIO_PACKET, BufferingRecovery, BufferingSpec, CuePayload, Fault, FlowStopReason,
            Pacing, StreamKind, StreamSpec, buffer_count, frame_pts,
        },
    };

    /// Re-check step while a push is parked on a stall gate. Only the test ends
    /// a stall. The step exists so flush and shutdown stay bounded.
    ...
    /// Downstream may briefly unlink the pad while it plugs elements, and
    /// sticky caps let a buffer land in that window. The bound is far above
    /// that window, so a never-linked pad still fails.
    ...
    static PAD_TEMPLATES: LazyLock<Vec<gst::PadTemplate>> = LazyLock::new(|| {
        vec![
            ...
            gst::PadTemplate::new(
                ...
                gst::PadDirection::Src,
                gst::PadPresence::Sometimes,
                // A SUPERSET of `caps::text_caps()`. A spec may override a text
                // stream's caps to something unreadable (`StreamSpec::caps_override`),
                // and a pad refuses caps its template does not cover.
                &caps::text_template_caps(),
            )
            .unwrap(),
        ]
    });
    ...
    enum Payload {
        ...
        Pattern(usize),
        Utf8(Vec<u8>),
        /// Verbatim bytes, pushed as given. A bitmap subtitle packet's meaning
        /// is entirely in the exact byte sequence.
        Bytes(Vec<u8>),
    }
    ...
    struct Buf {
        ...
        index: u64,
        pts: gst::ClockTime,
        /// `None` for a buffer that occupies no time of its own, the fragments
        /// of a bitmap cue, which share one pts and mean nothing apart. Dense
        /// media and text cues always carry one.
    ...
    fn build_schedule(spec: &StreamSpec) -> Vec<Item> {
        match &spec.kind {
                ...
                (0..count)
                    .map(|index| {
                        Item::Buffer(Buf {
                            ...
                            duration: Some(frame_pts(index + 1, *fps) - frame_pts(index, *fps)),
                            // ftestdec's needs-keyframe-after-flush knob depends on
                            // this exact rule.
            ...
            // Sparse stream. One buffer per cue carrying its text (so
            // bytes_per_buffer does not apply), GAP events over the rest. A
            // BITMAP cue is instead one buffer per chunk, all at the cue's
            // start (see `CueSpec::packets`), so the index is a running count
            // rather than the cue's position.
                    ...
                    match &cue.payload {
                        CuePayload::Text(text) => {
                            items.push(Item::Buffer(Buf {
                                index,
                                pts: cue.start,
                                duration: Some(end - cue.start),
                                keyframe: true,
                                payload: Payload::Utf8(text.clone().into_bytes()),
                            }));
                            index += 1;
                        }
                        CuePayload::Packets(chunks) => {
                            // The duration only on a lone chunk. A run of
                            // fragments sharing a pts does not each display for
                            // the cue's length.
                            let duration = (chunks.len() == 1).then(|| end - cue.start);
                            for chunk in chunks {
                                items.push(Item::Buffer(Buf {
                                    index,
                                    pts: cue.start,
    ...
    /// clips it. Past-the-end returns the schedule length, which the task
    /// turns into an immediate EOS. Video walks back to the nearest
    /// keyframe like a demuxer, so a decoder that needs one after a flush
    /// starts clean.
    fn start_index(schedule: &[Item], offset: gst::ClockTime, video: bool) -> usize {
        let Some(first) = schedule.iter().position(|item| match item {
            // A buffer with NO duration is instantaneous. It is reached by
            // a restart landing exactly on its pts, which `pts + 0 > offset`
            // would step past. Only bitmap fragments are in that shape.
            Item::Buffer(buf) => match buf.duration {
                Some(duration) => buf.pts + duration > offset,
                None => buf.pts >= offset,
            },
    ...
    fn make_buffer(buf: &Buf) -> gst::Buffer {
        let mut buffer = match &buf.payload {
            ...
            Payload::Bytes(bytes) => gst::Buffer::from_slice(bytes.clone()),
        };
        {
            let buffer = buffer.get_mut().unwrap();
            buffer.set_pts(buf.pts);
            if let Some(duration) = buf.duration {
    ...
    struct RunState {
        ...
        flushing: bool,
        stopping: bool,
        /// The schedule ended (EOS, an error, or an injected end). No further
        /// pushes until a reset.
        done: bool,
        position: gst::ClockTime,
        /// Where the current segment begins. Zero until a non-zero flushing
        /// seek restarts the schedule from an offset (see
        /// `handle_seek`).
        segment_start: gst::ClockTime,
        prng: Prng,
    }

    /// Hand-off from the streaming threads to the buffering worker. An anchored
    /// dip fires on the streaming thread, which must NEVER block on the
    /// bus, so it queues the recovery here and the worker does every post.
    ...
    /// The buffering worker and its queue, held while the media schedules
    /// buffering. The queue is created with the pads so every stream can
    /// reach it. The thread starts with the tasks.
    struct BufferingCtl {
    ...
    /// on its own thread. See [`BufferingSpec`] for the dip contract.
    fn run_buffering_schedule(
        ...
        // One full dip posts the low percent now and 100 when the recovery says
        // so. False when a stop or the element's end cut it short, after which
        // nothing further may be posted.
        let dip = |recovery: &BufferingRecovery| -> bool {
            ...
            if !recovered {
                // A lost 100 is invisible from a consumer's side, so name the reason.
                gst::debug!(
                    CAT,
        ...
        loop {
            // An anchored dip that fired goes first, then a due periodic tick. With
            // neither, the thread sleeps until woken.
            let request = {
                let mut queue = shared.queue.lock();
                loop {
                    if queue.stopping {
    ...
    /// harness's stand-in for a demuxer's flow combiner, which errors only
    /// when EVERY pad refuses, so one deselected stream must not kill the
    /// input while a sibling still plays. Single-stream inputs keep the
    /// immediate death.
    type FlowBoard = Arc<Mutex<Vec<Option<bool>>>>;

    /// One stream's schedule and streaming-task state.
    /// Upstream-selection mirror, shared by sibling streams of one media.
    /// SELECT_STREAMS arrives once per routed src pad, so the seqnum dedupes
    /// it, and `selected` models an adaptive demuxer's current track set
    /// (STREAMS_SELECTED posts only on change).
    #[derive(Default)]
    struct SelectState {
    ...
    /// streams as the collection with `selected_ids` marked selected.
    ...
    struct Stream {
        ...
        select: SelectBoard,
        /// Shared with the buffering worker, see [`BufferingShared`].
    ...
    impl Stream {
        ...
        /// Rewinds to the start of the schedule. A flush leaves STREAM_START
        /// and CAPS on the pad and only drops the segment, so only a
        /// fresh activation (`full`) re-sends them.
        fn reset(&self, full: bool) {
            self.reset_to(full, gst::ClockTime::ZERO);
        }

        /// [`reset`](Self::reset) at an offset. The next segment starts at
        /// `start` and the schedule resumes at [`start_index`]. Faults
        /// index buffers by schedule number, so one before the restart
        /// point never fires again, like a demuxer that never re-reads
        /// those bytes.
        ...
        /// Waits out `duration`. False when a flush or shutdown cut it short.
        ...
        fn park(&self, gate: &SyncPoint) -> bool {
            // Exactly once, since repeated blocking waits would inflate
            // SyncPoint::arrivals.
        ...
        /// free between steps so flushes and seeks can synchronise.
        fn iterate(&self, pad: &gst::Pad) {
            ...
            match step {
                None => {
                    // A paused task looks like a blocked one in a stack, and the three
                    // reasons differ (a flush resumes, `done` never does).
                    let state = self.state.lock();
                    gst::debug!(
                        CAT,
                        obj = pad,
        ...
        /// storing them, so each pending flag only clears once its
        /// event is actually on the pad. A buffer without them is an
        ...
        fn push_item(&self, pad: &gst::Pad, index: usize) {
            ...
            let duration = match item {
                Item::Buffer(buf) => buf.duration.unwrap_or(gst::ClockTime::ZERO),
            ...
            if let Item::Buffer(buf) = item {
                ...
                if let Some(reason) = flow_stopped {
                    // Deliberately the SAME message and debug shape as the unlinked
                    // give-up below. A consumer classifying on the debug text must not
                    // tell an injected race from a real one.
                    gst::debug!(
                        CAT,
                        obj = pad,
        ...
        /// Pushes one buffer, retrying while the pad has no peer. Only a flush
        /// or a shutdown ends the loop early, and only
        /// [`NOT_LINKED_BOUND`] turns NotLinked into the stream error a
            ...
            loop {
                match pad.push(make_buffer(buf)) {
                    Ok(_) => {
                        ...
                        self.advance(
                            index,
                            buf.pts + buf.duration.unwrap_or(gst::ClockTime::ZERO),
                        );
                        return;
                    }
                    ...
                    Err(gst::FlowError::Flushing) => {
                        // A flush is in progress, and it decides where we resume.
                        gst::debug!(
                            CAT,
                            obj = pad,
                    ...
                    Err(gst::FlowError::NotLinked) => {
                        // The bound expired. Aggregate like a demuxer's flow combiner
                        // first. While any SIBLING still delivers, this is a
                        // deselected stream and it idles rather than taking the whole
                        // input down.
                        ...
                        // Every pad refused, so post basesrc's EXACT shape. Consumers
                        // classify an external input's death by the "reason
                        // not-linked" debug text.
                        if let Some(element) = pad.parent_element() {
                            gst::element_error!(
                                element,
                                gst::StreamError::Failed,
        ...
        /// Runs on the streaming thread right after the push and NEVER
        /// blocks. A schedule restarted behind the index fires the dip
        /// again, exactly like a fault.
        ...
        fn handle_event(self: &Arc<Self>, pad: &gst::Pad, event: gst::Event) -> bool {
            match event.view() {
                ...
                gst::EventView::FlushStop(_) => {
                    // The core holds the streaming lock here, so the task is between
                    // steps and the sticky segment is already gone.
                    self.reset(false);
        ...
        /// Adaptive-demuxer SELECT_STREAMS semantics. One unrecognized id
        /// rejects the WHOLE event, a fully-known set applies, and
        /// STREAMS_SELECTED posts only when the selected set changed. The
        /// event arrives once per routed src pad, deduped by seqnum.
        ...
        /// target (see [`start_index`]) and the new segment begins
        /// there. Other rates are refused, the schedule has no trick
        /// modes.
        fn handle_seek(self: &Arc<Self>, pad: &gst::Pad, seek: &gst::event::Seek) -> bool {
            ...
            // SeekType::None means "restart from the beginning" here, not "keep the
            // current position".
        ...
        fn handle_query(
            ...
            match query.view_mut() {
                ...
                // decodebin3 asks at STREAM_START. Answering TRUE flips it into
                // upstream-selection mode for the WHOLE element.
                gst::QueryViewMut::Selectable(q) if self.scenario.spec().upstream_selection => {
    ...
    impl FTestSrc {
        ...
        /// Resolves the scenario the URI points at. A missing one is a bus
        /// error, not a panic, so a test typo stays readable.
        ...
        /// Adds every stream's pad while still in READY, so downstream is
        /// linked before the parent activates the pads and the tasks
        /// start pushing.
        fn create_pads(&self) -> Result<(), gst::StateChangeError> {
            ...
            let flows: FlowBoard = Arc::new(Mutex::new(vec![None; scenario.spec().streams.len()]));
            // The queue exists before any stream so every task can reach it. The
            // worker starts with the tasks, see start_buffering.
            self.stop_buffering();
                ...
                let pad = gst::Pad::builder_from_template(template)
                    ...
                    .activatemode_function({
                        ...
                        move |pad, _parent, mode, active| {
                            ...
                            if active {
                                // Pads are activated inside add_pad, before pad-added
                                // ran and downstream got linked, so the tasks start
                                // later, see start_tasks.
                                Ok(())
                            } else {
                                stream.request_stop();
        ...
        /// Starts pushing once every pad is added and linked, the way a demuxer
        /// only pushes after pad-added and no-more-pads.
        fn start_tasks(&self) -> Result<(), gst::StateChangeError> {
            ...
            // An adaptive demuxer announces its initial selection when its tracks
            // activate. The caller's engine seeds from that post.
        ...
        /// Starts the buffering worker when the media schedules buffering. Runs
        /// after the pads are added and linked, so the initial low post
        /// lands during the pipeline's first preroll.
        ...
        /// Stops and joins the buffering worker. It re-checks its queue at
        /// least every [`STALL_POLL`], so the join is bounded even
    ...
    impl ElementImpl for FTestSrc {
        ...
        fn send_event(&self, event: gst::Event) -> bool {
            // The default implementation picks one random src pad. A seek has to reach
            // every stream.
        ...
        fn change_state(
            ...
            match transition {
                ...
                gst::StateChange::ReadyToNull => {
                    // Covers a ReadyToPaused that failed before the tasks started,
                    // where PausedToReady never runs.
                    self.stop_buffering();
