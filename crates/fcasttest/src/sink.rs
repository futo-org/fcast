//! `ftestsink`: recording sink and sequence assertions.
//!
//! Records every buffer, event and state transition into one ordered log while
//! changing nothing about the base class data path: no vmethod drops or
//! reorders anything, no clock is provided, and `sync` keeps the BaseSink
//! default.
...
/// Canonical `GstEventType` names, as stored in
/// [`RecordEntry::Event::type_name`].
pub mod event_name {
...
/// One observation, in arrival order. Buffer entries carry metadata only, never
/// a payload reference, so a log keeps no buffer alive.
#[derive(Clone, Debug)]
pub enum RecordEntry {
    ...
    /// The preroll buffer, from the GstBaseSink `preroll` vfunc. The same
    ...
    Event {
        ...
        seqnum: gst::Seqnum,
        sticky: bool,
        /// Only filled for events the harness cares about, and only while
        /// `capture-details` is set.
...
impl RecordEntry {
    ...
    /// preroll entry included. It is the same buffer, so its flags are read
    /// at render.
    ...
    /// [`Self::synthetic_buffer`] with flags, see
...
impl Recording {
    ...
    /// Appends an entry and wakes every waiter. Public so tests and the fuzz
    /// driver can assemble synthetic logs through the same type.
    ...
    /// Drops every entry, counters included. A sink never clears by itself, not
    /// even across a state change, so a log survives teardown.
    pub fn clear(&self) {
        let mut log = self.inner.log.lock();
        log.entries.clear();
        log.buffers = 0;
    ...
    /// Blocks until `pred` accepts the log or `timeout` expires. `pred` runs
    /// UNDER the log lock and must not call back into this handle.
...
/// Splits a `recording-key` into scenario key and handle name: `"scen1"`
/// stashes under [`DEFAULT_HANDLE_NAME`], `"scen1/audio"` under `"audio"`.
...
/// Sequence invariants over a recorded (or synthetic) log. Every checker is
/// total and reports the first violation with the entry indices that prove it.
pub mod asserts {
    use super::{RecordEntry, event_name};

    /// Runs every invariant that holds for any sink under any pipeline.
    /// [`first_buffer_is_discont`] is deliberately excluded, see its own docs.
    ...
    /// Timed data: a buffer, a preroll, or the GAP that stands in for one. All
    /// three are positioned by the segment and refused by an EOS or
    /// flushing pad, so every "data" rule covers the GAP too.
    fn is_timed_data(entry: &RecordEntry) -> bool {
        entry.is_data() || entry.is_event(event_name::GAP)
    }

    /// Serialized events describing the stream rather than a moment in it. Each
    /// travels the pad under the stream lock, so a flushing pad refuses it and
    /// an EOS pad discards it, exactly like a buffer.
    ///
    /// FLUSH_START/FLUSH_STOP are excluded (they open and close a flush, and
    /// FLUSH_STOP is the one serialized event a flushing pad accepts), as is
    /// STREAM_START, which CLEARS an EOS but is still refused mid-flush. The
    /// two callers filter it themselves.
    ...
    /// Every FLUSH_START is followed by a FLUSH_STOP before the next one and
    /// before the log ends. A stray FLUSH_STOP is legal. The log may begin
    /// (or have been cleared) mid-flush.
    ...
    /// A CAPS event precedes the first buffer. Caps stay sticky across a flush,
    /// so only the log's first buffer is checked.
    ///
    /// A GAP has to count. A sparse stream opens with one rather than a
    /// buffer, which a buffer-only rule never inspects.
    ...
    /// No buffer or preroll whose PTS is behind the one before it, within a
    /// segment. A sink renders in arrival order, so a PTS moving backwards
    /// is a stream it cannot present as received.
    ///
    /// Reset by SEGMENT, FLUSH_STOP and STREAM_START, the three ways the
    /// timeline legally restarts. A buffer with no PTS is skipped, not
    /// treated as zero.
    ///
    /// GAP is out of scope only because [`RecordEntry::Event`] keeps a gap's
    /// timestamps in the `details` string. Widen the entry before this rule, do
    ...
    /// No buffer arrives after an EOS. STREAM_START and FLUSH_STOP clear the
    /// EOS state as GstBaseSink does. GAP counts as data, because a pad
    /// discards every serialized event once it is EOS.
    ...
    /// Nothing describing the stream reaches an already-EOS pad either. The
    /// pad discards every serialized event there, so a CAPS, SEGMENT or
    /// STREAM_COLLECTION recorded after an EOS did not travel a correct pad.
    ///
    /// Companion to [`no_buffer_after_eos`], clearing on the same events. It
    /// catches misordered re-caps and re-segments against an outgoing EOS
    /// that leave no buffer out of place for the data rule.
    ...
    /// One EOS per stream. An already-EOS pad drops the sticky event, so a
    /// second EOS with no STREAM_START or FLUSH_STOP between them cannot
    /// reach a sink through a pad.
    ...
    /// A flushing pad refuses data, so a FLUSH_START/FLUSH_STOP window must be
    /// empty of buffers, prerolls and gaps.
    ///
    /// One exception, which is why this counts rather than flags. A chain call
    /// already past the flushing check holds the stream lock and still
    /// finishes, so exactly ONE in-flight render can be recorded after the
    /// FLUSH_START. A SECOND entry means data really did flow through a
    /// flush.
    pub fn no_data_during_flush(log: &[RecordEntry]) -> Result<(), String> {
        /// See [`no_data_during_flush`], the render already in flight.
    ...
    /// whole window shares ONE in-flight slot.
    ///
    /// The stream lock is held for the whole of a chain call or a serialized
    /// event, so at most one dispatch of any kind is ever past the flushing
    /// check. That shared slot is the point of this checker. Data alone and
    /// events alone are each already bounded at one, but a buffer AND an
    /// EOS in one window passes both of those rules and is still
    /// impossible.
    ///
    /// FLUSH_STOP is the terminator, the one serialized event a flushing pad
    /// accepts. StateChange entries are exempt. They never touch the pad,
    /// and a flushing seek across a state change legitimately puts one
    /// here.
    ...
    /// A STREAM_START precedes the first SEGMENT, as it does the first CAPS.
    /// Sticky events travel a pad in sticky order, so a segment ahead of
    /// the stream-start is a misordering GStreamer itself warns about.
    ///
    /// Without this, a log carrying no caps event at all (or one after the
    /// segment) slips past the caps rule entirely.
    ...
    /// GST_BUFFER_FLAG_DISCONT. Decoder base classes start with `discont` set
    /// and clear it on the first output.
    ///
    /// DELIBERATELY NOT IN [`all`]. Only this weakest form is measured.
    /// "Only the first buffer is DISCONT" is FALSE (a starved realtime source
    /// really does produce more), and the stronger per-STREAM_START and
    /// post-FLUSH_STOP forms are unvalidated here. Measure first, then widen.
    ///
    /// Skipped when the log does not open with a STREAM_START ahead of its
    /// first buffer. That log was cleared or attached mid-stream.
...
mod imp {
    ...
    pub struct FTestSink {
        ...
        /// Per-entry gst logging, off by default so the record path never
        /// formats.
        silent: AtomicBool,
        ...
        stall_ms: AtomicU64,
        /// Thread that entered the stall, published through the read-only
        /// `stalled-thread` property. Empty until it engages, which is how a
        /// test detects that it engaged at all.
    ...
    impl FTestSink {
        ...
        /// False when nothing is registered under the key yet, which is
    ...
    impl ObjectSubclass for FTestSink {
        ...
        fn class_init(klass: &mut Self::Class) {
            // gstreamer-rs does not wrap the GstBaseSink preroll vfunc, so install it
            // by hand. SAFETY: Self::Class is #[repr(C)] with GstBaseSinkClass first.
    ...
    impl ElementImpl for FTestSink {
        ...
        fn change_state(
            ...
            // Before chaining up, so the entry precedes whatever the transition sets
            // in motion.
            self.record(RecordEntry::StateChange {
                transition,
                monotonic: Instant::now(),
            });
            // Manufactured slow state change, see `stall-transition`. Held BEFORE
            // chaining up, so the caller of `set_state` waits inside the element's own
            // transition. Once only, or the release path would stall too.
            ...
            if stall {
                // The OS thread name, not the Rust one. The threads that matter are
                // GStreamer's, which Rust reports as unnamed. `/proc/thread-self` is
                // the CURRENT thread's directory, so this names the caller exactly.
            ...
            if transition == gst::StateChange::NullToReady && !self.stash_recording() {
                // Retried because a scenario may be registered after the property is
                // set. Still missing by now is a test bug.
                gst::warning!(
                    CAT,
                    imp = self,
    ...
    impl BaseSinkImpl for FTestSink {
        ...
        fn event(&self, event: gst::Event) -> bool {
            // Before chaining up. The parent drains and posts on EOS, and resets on
            // flush-stop, both without returning here.
    ...
    /// Detail strings for the events the harness reasons about. Everything else
...
impl FTestSink {
    ...
    /// The log this sink records into. Cheap to clone, shared with every
    /// handle.
    pub fn recording(&self) -> Recording {
...
/// Rank NONE so tests select the sink explicitly and autoplugging never does.
