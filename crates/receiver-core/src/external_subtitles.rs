...
//! out.
//!
//! External subtitles are advertised to senders as ordinary subtitle tracks, so
//! they need ids in the same namespace as the embedded tracks. Embedded ids are
//! positions in `Player::streams` (kept stable across collections by
//! `player::merge_streams_stable`); externals live above
//! [`EXTERNAL_TRACK_ID_BASE`] and must be independent of BOTH the list position
//! of their stream and their position in this catalog. A removal must not
//! renumber the survivors: senders cache advertised ids, and a renumbering makes
//! an in-flight `SetTrack` name the wrong track or nothing at all.
//!
//! The invariant is therefore structural rather than a convention spread over
//! the call sites: an id is assigned exactly once, at attach time, from a
//! monotonic counter that a removal never rewinds, and every advertise and
//! resolve path goes through this catalog. The counter survives
//! [`clear`](Catalog::clear) (a queue advance inside one media source), so an id
//! is never reused while the receiver plays through that source.
//!
//! Ids are receiver-assigned opaque integers on the wire, so none of this is a
//! protocol concern. The FCast schema is frozen and untouched.
...
/// a pipeline (the real handle can only come from a live playbin).
...
pub(crate) struct ExternalSubtitle<H = SubHandle> {
    ...
    /// (>= [`EXTERNAL_TRACK_ID_BASE`]). Assigned once at attach and never
    /// reassigned or reused, so removing a sibling entry cannot renumber it.
    pub(crate) id: u32,
    ...
    /// attached simultaneously, selection is pure SELECT_STREAMS). The handle
    /// is stable for the entry's whole life: fcastplaybin re-arms a dead
    /// deselected input internally under the same id, and a genuine failure
    /// comes back as `PlayerEvent::ExternalSubtitleFailed`, which removes the
    /// entry.
    pub(crate) handle: H,
    ...
    /// fcastplaybin's internal input replacements. All id/stream mapping goes
    /// through this, never through the live handle.
...
pub(crate) struct Catalog<H = SubHandle> {
    ...
    /// [`clear`](Self::clear), so an id is never reused.
    next_id: u32,
}
...
impl<H: Copy + PartialEq> Catalog<H> {
    ...
    pub(crate) fn attach(
        ...
        // Saturating rather than wrapping: wrapping would alias a live id, and
        // the receiver would have to attach 268 million subtitles to one media
        // source to get here.
    ...
    /// Every entry, in stable catalog order (what TracksAvailable advertises
    /// after the embedded tracks).
    ...
    /// The stable track id of the entry whose stream is `sid`, for mapping a
    /// selection report back onto an advertised id. `None` means the stream
    /// belongs to no catalog entry, so it is an embedded track.
    ...
    /// list. Reading it from the same field [`id_of_stream`](Self::id_of_stream)
    /// uses is what keeps TracksAvailable and TracksSelected from disagreeing
    /// about which streams are externals.
    ...
    /// Learn the stream id of every entry whose stream just materialized.
    /// `lookup` answers for an attached input once decodebin3 advertises its
    /// stream. Returns what it resolved, for logging. Already-known sids are
    /// never overwritten (the first one is the stable, URI-derived answer).
    ...
    /// stays retired for the rest of the media source (it resolves to nothing,
    /// the correct answer for a track that no longer exists).
...
mod tests {
    ...
    /// which is exactly why the catalog is generic over the handle.
    ...
    fn ids_start_at_the_base_and_count_up() {
        ...
        // Every assigned id is recognizable as external, and no embedded track
        // index can reach the namespace.
    ...
    /// The defect this structure exists to prevent: removing one external must
    /// not renumber the survivors, or a sender's cached or in-flight SetTrack
    /// names the wrong track.
    #[test]
    ...
    /// The removed id resolves to nothing, which is the correct answer for a
    /// track that no longer exists (the caller turns that into an error reply).
    #[test]
    ...
    /// Clearing and attaching again must not hand out a used id either (a queue
    /// advance inside one media source clears the catalog).
    #[test]
    ...
    /// A fresh media source starts the numbering over (the catalog is per item,
    /// and senders re-read TracksAvailable for every load).
    #[test]
    ...
    fn stream_ids_map_back_to_the_stable_track_id() {
        ...
        // The exclusion list the embedded advertising uses reads the same
        // field, so the two sides cannot disagree about which streams are
        // advertised as externals.
        let materialized: Vec<&str> = catalog.materialized_sids().collect();
        assert_eq!(materialized, ["sub-c"]);
    }

    /// A second learn pass must not re-report or overwrite what is already
    /// known (it runs on every collection).
    #[test]
    fn learning_a_stream_id_is_idempotent() {
        ...
        // A later answer with a different sid (an input replacement) does not
        // move the mapping, and nothing is re-reported.
        assert!(
            catalog
