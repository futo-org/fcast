//! AirPlay NTP timing service: the receiver is the *active* peer, polling the
//! `timingPort` the client advertised in `SETUP` to estimate the offset between
//! the client's ("remote") wall clock and ours (port of UxPlay's `raop_ntp.c`).
//!
...
//! we adopt the offset of the lowest-delay of the last [`NTP_DATA_COUNT`]
//! samples.
//!
//! This clock does not yet drive buffer PTS: tight lip-sync additionally needs
//! the audio RTCP sync packets and a `CLOCK_REALTIME` -> running-time mapping.
...
/// Drift-corrected mapping between the client's ("remote") and our local wall
/// clock, in ns; clones share state. `offset` is `remote - local`, and `None`
...
impl NtpClock {
    ...
    /// Remote timestamp (ns) to our local wall clock (ns since the Unix epoch),
    /// or `None` if not yet synced.
...
/// Inverse of [`put_ntp_timestamp`], back to ns since 1970.
...
/// NTP-format u64 (high 32 = seconds, low 32 = fraction) to ns. With
/// `ntp_epoch` the seconds are relative to 1900; otherwise they are a bare
/// monotonic since-boot counter, as in the mirror video timestamps.
...
/// The offset/delay sample for one exchange.
...
/// The offset of the lowest-delay sample so far.
fn best_offset(samples: &[Sample]) -> Option<i64> {
    samples.iter().min_by_key(|s| s.delay).map(|s| s.offset)
}

/// Poll `timing_addr` until aborted, updating `clock` after every reply.
...
pub async fn run(socket: UdpSocket, timing_addr: SocketAddr, clock: NtpClock, device_name: String) {
    ...
    loop {
        ...
        // The mirror timing clock carries the 1900 epoch.
...
/// The client timing address: peer IP plus the `SETUP` `timingPort`.
