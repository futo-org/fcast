...
//! fire exactly once (measured: one bus error at ~5s, then the writer parks
//! instead of crawling):
//!
//! ```text
...
//! seconds). Both of these RECOVER, which is itself the finding: removing the
//! target node makes the session manager re-link us, and suspending it makes
//! the stream re-negotiate.
//!
//! ```text
...
fn silence_through_a_live_pipewire_graph() {
    ...
    if expect_stall {
        // The stall must be reported, ONCE: a refused segment is skipped and
        // written straight back, so an unlatched report floods the bus.
