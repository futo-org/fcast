...
mod imp {
    ...
    // --- PICOLA tuning, ported verbatim from streamcraft's audiostretch ------------------

    /// Pitch range the period search covers. 65 Hz reaches a deep male voice, 400 Hz a high
    /// one. Unvoiced or silent stretches have no true period; the search still returns some
    /// minimum, which splices fine because noise has no phase to misalign.
    const MIN_PITCH_HZ: usize = 65;
    const MAX_PITCH_HZ: usize = 400;
    /// The AMDF coarse pass runs on a mixdown decimated to roughly this rate, then the winner
    /// is refined at full resolution.
    ...
    /// Rate clamp. Outside this the algorithm stops being sane; scaletempo has no clamp but
    /// also has nothing sensible to say at 100x.
    const RATE_MIN: f64 = 0.25;
    const RATE_MAX: f64 = 4.0;

    // --- The PICOLA engine ----------------------------------------------------------------

    /// Fixed-capacity input buffer, period search, splice schedule. Built once the format is
    /// known and rebuilt if it changes. `generate` never allocates.
    struct Engine {
        channels: usize,
        min_period: usize,
        max_period: usize,
        ...
        /// Last measured period and its AMDF score. Reused through silence, and preferred over
        /// a new measurement that matches *worse* than the last one did — unvoiced frames have
        /// no true period, and letting their random minima through jitters the splice cadence.
        prev_period: usize,
        prev_min_diff: f32,
        /// Scratch: mono mixdown of the search window, and its decimated copy.
        mono: Vec<f32>,
        mono_coarse: Vec<f32>,
        /// Fractional frames of skip/insert owed, carried between splices so the long-run
        /// tempo is exact.
        frac: f64,
    ...
    impl Engine {
        fn new(rate_hz: usize, channels: usize) -> Self {
            ...
            // Room for a full search window plus a feed chunk, so a push can always make
            // progress after a drain without the buffer reallocating.
            let buf_frames = window + 2 * COPY_CHUNK;
            // Worst-case round output: the EOS tail passes through up to a whole window, a
            // slow-down round emits `2 * period`, a copy round emits COPY_CHUNK.
        ...
        /// Append interleaved f32 frames. The caller must respect [`Engine::free_frames`].
        fn push(&mut self, samples: &[f32]) {
            self.buf.extend_from_slice(samples);
        }

        /// AMDF pitch period of the input at the read head: the lag in
        /// `[min_period, max_period]` minimising the mean `|s[i] - s[i+lag]|`. Coarse pass on a
        /// decimated mixdown, refined at full resolution.
        ///
        /// (AMDF period detection is Ross et al., "Average magnitude difference function pitch
        /// extractor", IEEE Trans. ASSP 22(5), 1974.)
        fn find_pitch_period(&mut self) -> usize {
            ...
            // Silence carries no period; keep the speaker's last one so pauses splice at the
            // same cadence.
            ...
            // A match that fits worse than the previous frame's did is likely an unvoiced
            // stretch; keep the established period instead.
            ...
            // A non-finite score (NaN input) would make every later comparison false and
            // freeze the hysteresis. Unreachable for finite audio.
            self.prev_min_diff = if best_diff.is_finite() { best_diff } else { f32::MAX };
            chosen.clamp(1, self.max_period)
        }

        /// Emit `frames` frames crossfading `from` (fading out) into `to` (fading in), both
        /// frame offsets relative to the read head. The linear ramp spans the whole splice, one
        /// pitch period, so joins are gentle. Linear rather than equal-power is correct here
        /// precisely because the two sides are phase-aligned: correlated signals sum linearly.
        fn overlap_add(&mut self, frames: usize, from: usize, to: usize) {
            let ch = self.channels;
            let base = self.start;
            // The splice schedule never asks for more than `2 * max_period` frames and a round
            // only runs with `3 * max_period` buffered, so this clamp is unreachable. It is
        ...
        /// One round: a verbatim run or one pitch-synchronous splice, appended to `out`.
        /// Returns false when the round cannot run yet (needs more input). `eos` unlocks the
        /// short-input tail branch.
        fn generate(&mut self, scale: f64, eos: bool) -> bool {
            // Scrub a NaN carried in from non-finite input, which would otherwise turn every
            // later `ideal`/`copy` into NaN and pin the splice length at its clamp forever.
            if !self.frac.is_finite() {
                self.frac = 0.0;
            }
            let buffered = self.buffered_frames();

            // Unity (or near-unity) scale is a verbatim forward. This branch also guards the
            // slow-down schedule below, whose `1.0 - scale` denominator is zero at exactly 1.0.
            ...
            if buffered < self.window {
                ...
                // Tail end of the stream: pass the remainder through. Tempo drifts for the
                // final few tens of ms, which is inaudible.
            ...
            if scale > 1.0 {
                // Speed up: blend one period into the next, dropping a period. The blend
                // length shrinks as the rate grows (Sonic's schedule): at exactly 2x every
                // period is spliced with its neighbour, above that the splice itself must span
                // less than a period.
                if scale >= 2.0 {
                ...
                // output the input does not advance through.
                let ch = self.channels;
                if scale < 0.5 {
    ...
    struct State {
        ...
        /// Output timeline anchor, set from the first buffer after a segment or flush.
    ...
    pub struct AudioStretch {
        ...
        /// Scale taken from the segment, kept outside `state` so a SEGMENT arriving before
        /// caps is not lost.
    ...
    impl Default for AudioStretch {
        fn default() -> Self {
            // Explicit rather than derived: `f64::default()` is 0.0, and a scale of 0.0 reaching
            // the engine takes the slow-down branch with a zero numerator, stretching one input
            // frame into a whole period indefinitely. Caps negotiate before the first SEGMENT,
            // so this really is the value `set_caps` reads on the first configure.
            Self { state: Mutex::new(None), pending: Mutex::new((1.0, false)) }
        }
    }

    impl AudioStretch {
        /// Run rounds until the engine stalls, returning the produced buffer if any.
        ///
        /// `engine.out` is deliberately *not* cleared on entry: `submit_input_buffer` also runs
        /// rounds when the fixed-capacity queue fills, and clearing here would silently discard
        /// that output. It is cleared only after being turned into a buffer.
        fn drain(&self, state: &mut State, eos: bool) -> Option<gst::Buffer> {
            ...
            while state.engine.generate(state.scale, eos) {
                // A round is bounded; keep going until it cannot make progress.
            ...
            let mut buffer = gst::Buffer::with_size(frames as usize * bpf).ok()?;
            {
                ...
                let pts = base
                    + gst::ClockTime::from_nseconds(
                        state.emitted.saturating_mul(gst::ClockTime::SECOND.nseconds()) / rate,
                    );
                let end = base
                    + gst::ClockTime::from_nseconds(
    ...
    impl BaseTransformImpl for AudioStretch {
        ...
        fn sink_event(&self, event: gst::Event) -> bool {
            ...
            match event.view() {
                EventView::Segment(e) => {
                    ...
                    let scale = if bypass { 1.0 } else { scale.clamp(RATE_MIN, RATE_MAX) };

                    *self.pending.lock().unwrap() = (scale, reverse);

                    // A SEGMENT can arrive *without* a preceding flush — that is exactly what a
                    // gapless track transition looks like. Resetting the engine unconditionally
                    // would discard whatever it still holds (up to one search window, ~46 ms),
                    // silently swallowing the tail of the outgoing track. Drain it first; on a
                    // flushing seek FLUSH_STOP has already emptied the engine, so this is a
                    // no-op there.
                    let tail = {
                    ...
                    // Consume the rate: downstream plays at 1.0 over audio we compressed.
                    // `stop` shrinks by the same factor so the segment still describes the
                    // material we will actually emit. Mirrors gst_scaletempo_sink_event.
                ...
                EventView::Eos(_) => {
                    // Drain whatever the engine still holds before letting EOS through, or the
        ...
        fn submit_input_buffer(
            ...
            // NOTE: `is_discont` must NOT reset the engine, however tempting that looks.
            //
            // `GstBaseTransform` clears its `priv->discont` flag only when an output buffer is
            // actually pushed (gstbasetransform.c:2414-2421). This element needs a whole search
            // window (~46 ms) buffered before it can emit anything, so resetting here creates a
            // self-sustaining livelock: flush sets discont, we wipe the queue, we cannot reach
            // the window, no buffer is pushed, the flag stays set, and the next buffer wipes the
            // queue again — forever. That is the rate-change stall from `rate-change-stuck.txt`:
            // audio flows in, nothing comes out, and the sink never prerolls.
            //
            // `is_discont` is a "mark the next output buffer DISCONT" signal, which BaseTransform
            // applies itself. The discontinuities that genuinely require dropping buffered audio
            // arrive as FLUSH_STOP and SEGMENT, and both are handled in `sink_event`.
            let _ = is_discont;

            // Anchor the output timeline on the first buffer of a segment, mapping its stream
            // time into the compressed timeline the way scaletempo's per-buffer formula does.
            ...
            // Feed in chunks the engine has room for, running rounds in between so a large
            // input buffer cannot overrun the fixed-capacity queue.
            let bps = if format == gst_audio::AUDIO_FORMAT_S16 { 2 } else { 4 };
            ...
            while offset < data.len() {
                ...
                if room == 0 {
                    // Engine is full and cannot take more; a round must run first.
