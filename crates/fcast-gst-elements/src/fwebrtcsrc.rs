...
/// What the signaller hands back to whoever is driving the mirroring session.
///
/// Defined here rather than in the receiver so the element and its tests build
/// without it. The receiver re-exports both types from its `fcast` module.
pub enum InternalMessage {
    Answer { sdp: String },
}

/// The offer side of the same contract. The session pushes remote SDP offers
/// in, the signaller takes the receiver out once and consumes it.
...
pub struct SignallingChannel {
    pub tx: tokio::sync::mpsc::UnboundedSender<InternalMessage>,
    pub offer_rx: MirroringOfferRx,
}
    ...
    impl FSignaller {
        async fn send_answer(&self, webrtcbin: gst::Element) {
            ...
            if let Some(chan) = settings.channel.as_ref() {
                if let Err(e) = chan
                    .tx
                    .send(super::InternalMessage::Answer { sdp: answer_sdp })
                {
        ...
        async fn on_ice_gathering_complete(&self, webrtcbin: gst::Element) {
            ...
            // Only answer while actively negotiating, so a terminated session emits no
            // stale answer.
            match state {
        ...
        pub fn on_webrtcbin_ready(&self) -> RustClosure {
            glib::closure!(|signaller: &super::FSignaller,
                ...
                gst::debug!(CAT, obj = signaller, "Webrtcbin ready");
    ...
    impl SignallableImpl for FSignaller {
        fn start(&self) {
            *self.state.lock() = State::Negotiating;
            ...
            fcast_runtime::RUNTIME.spawn(async move {
