...
pub mod imp {
    ...
    impl ObjectImpl for FVaJpegDec {
        fn constructed(&self) {
            ...
            // Never panic here: on failure the bin is left without pads and decodebin3
            // autoplug falls back to fimagedec.
    ...
    impl FVaJpegDec {
        /// Assemble `jpegparse ! vajpegdec` plus ghost pads and probes; `None`
        /// (never a panic) if any element or pad is unavailable.
        fn try_build(&self) -> Option<()> {
            ...
            // Relabel the CAPS event image/x-fcast-jpeg -> image/jpeg. The probe fires
            // before jpegparse's own event handler, so the rewritten caps is
            // what it validates.
            ...
            dec_src.add_probe(gst::PadProbeType::EVENT_DOWNSTREAM, move |_pad, info| {
                ...
                match event.view() {
                    ...
                    // A still is held forever: swallow EOS so the pipeline stays PLAYING on it.
                    gst::EventView::Eos(_) => gst::PadProbeReturn::Drop,
                    _ => gst::PadProbeReturn::Ok,
                }
            });

            // The sink advertises the private image/x-fcast-jpeg but jpegparse only accepts
            // image/jpeg, so caps must be translated in BOTH directions: CAPS/ACCEPT_CAPS
            // queries are answered here from the template (never proxied down
            // to jpegparse), the downstream CAPS event is rewritten by the
            // probe above. Deliberately no `identity` in front: a passthrough
            // would proxy ACCEPT_CAPS straight to jpegparse and be rejected.
...
pub fn plugin_init() -> Result<(), glib::BoolError> {
    ...
    // Only register when the whole chain can be built, so the element is never
    // instantiated (and thus never crashes) on a box where baseline JPEG has to
    // stay on fimagedec.
...
/// Whether `vajpegdec` exists and can reach READY (VA display plus a JPEG
/// decode entrypoint).
#[cfg(target_os = "linux")]
...
mod tests {
    ...
    fn registers_when_dependencies_present() {
        ...
        // Another test in this module may already have registered it.
    ...
    /// A baseline JPEG must decode through `fvajpegdec`, not fall back to
    /// `fimagedec`: exercises ACCEPT_CAPS plus the caps event across the
    /// image/x-fcast-jpeg -> image/jpeg boundary.
    #[test]
