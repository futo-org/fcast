...
use fcast_video::{cue::CueEngine, cue_ir::VideoRect, video::Rotation};
...
/// The displayed size of a coded frame under the container's rotation. A
/// quarter turn trades width for height, which is what the presenters
/// letterbox with.
#[allow(dead_code)]
pub(crate) fn turned(rotation: Rotation, width: u32, height: u32) -> (u32, u32) {
    match rotation {
...
mod cue_geometry_tests {
    ...
    use fcast_video::{
        ...
        cue_ir::{CueIr, ir},
        video::{Overlay, Rotation},
    };

    use super::{CueGeometry, turned, video_rect};
    ...
    /// recorded sideways letterboxes as the portrait it plays as.
    #[test]
    fn a_rotated_clip_fits_by_its_turned_size() {
        assert_eq!(turned(Rotation::Rotate90, 1920, 1080), (1080, 1920));
        assert_eq!(turned(Rotation::Rotate270, 1920, 1080), (1080, 1920));
        assert_eq!(turned(Rotation::Rotate180, 1920, 1080), (1920, 1080));

        let upright = video_rect((1920, 1080), WINDOW).expect("real sizes");
        assert_eq!((upright.x, upright.width), (0, 1920));
        let turned_rect =
            video_rect(turned(Rotation::Rotate90, 1920, 1080), WINDOW).expect("real sizes");
