//! Zero-copy import of VideoToolbox IOSurface-backed frames into libplacebo GL
//! textures.
//!
//! `vtdec` decodes into IOSurface-backed CVPixelBuffers and (given the
//! `memory:IOSurface` caps feature) hands them downstream without a CPU
//! readback. This module borrows the `IOSurfaceRef` out of a `GstMemory` and
//! wraps each plane in a `GL_TEXTURE_RECTANGLE` via `CGLTexImageIOSurface2D`,
//! the exact call GStreamer's own `iosurfaceglmemory.c` makes. The
//! resulting texture is then wrapped by libplacebo with `pl_opengl_wrap` (see
//! `placebo.rs`).
//!
//! This mirrors the Linux dma-buf path (`dmabuf.rs` +
//! `placebo::render_dmabuf`): the buffer carries a kernel-shareable handle that
//! the app imports into its own GL context at render time.
//!
//! The `gst_iosurface_*` symbols come from `libgstiosurface-1.0` (unstable
//! GStreamer API, "Since: 1.30"). We hand-declare them because no
//! `gstreamer-iosurface` crate exists for the 0.25 series. If the pinned
//! GStreamer ref is ever bumped, re-diff `gstiosurface.h` against these
//! declarations.
...
/// Caps feature string for IOSurface-backed memory
...
// GL enum constants (macOS OpenGL headers). Declared here rather than pulled
// from `glow` because the import path uses the CGL C API directly and never
// touches the `glow::Context`.
type GLenum = c_uint;
type GLint = i32;
type GLuint = c_uint;
type GLsizei = i32;
...
/// GL format table for the plane of a given video format. `None` for
/// formats/planes the importer does not implement (caller must fall back to
/// system memory). Mirrors the table in the plan.
...
/// Borrow the `IOSurfaceRef` (and its plane index) out of an IOSurface-backed
/// `GstMemory`. The returned surface is owned by `mem` and stays valid as long
/// as `mem` (and therefore the owning `gst::Buffer`) is alive.
...
/// A GL texture (`GL_TEXTURE_RECTANGLE`) bound to one IOSurface plane. Deletes
/// the texture on drop. The libplacebo `pl_tex` wrapper created from `id` does
/// *not* own the GL object, so this guard must outlive the wrapper and be
/// dropped only after `pl_tex_destroy`.
pub struct PlaneTexture {
    pub id: GLuint,
    /// Plane dimensions in texels (device-aligned; from the IOSurface, not the
    /// video info).
    pub width: i32,
    pub height: i32,
...
impl PlaneTexture {
    /// The `GL_TEXTURE_RECTANGLE` target the texture is bound to (for
...
/// Import one plane of `surface` into a fresh `GL_TEXTURE_RECTANGLE`, using the
/// currently-current CGL context (Slint's, when called from
/// `SwapchainSink::render`). Returns `None` if there is no current context or
/// the CGL import fails.
///
/// # Safety
///
/// A CGL/OpenGL context must be current on the calling thread and `surface`
/// must be a live IOSurface with at least `plane + 1` planes.
pub unsafe fn import_plane(
    ...
    unsafe {
        ...
        if err != 0 {
            // Log the exact CGL error rather than a generic import failure.
            // `tex` drops here, freeing the GL texture.
            tracing::warn!(
                cgl_error = err,
