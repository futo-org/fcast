...
//! fcastplaybin.
//!
//! Adding a regression case is adding a file. The trade is depth. A file
//! describes media and the smoke run asserts what holds for all media (it
//! loads, it plays, every sink sequence is legal, the worker still answers).
//! A scenario that needs an action schedule belongs in `tests/scenarios.rs`
//! where a timeline can drive it.
//!
//! Discovery is runtime, so one failing file names itself and the rest still
//! run.
...
fn init() {
    ...
    INIT.call_once(|| {
        ...
        // fcastaudiostretch is built by the fcastplaybin constructor but
...
/// The `tests/scenarios.rs` harness, trimmed to what a smoke run needs:
/// recording sinks, the pumped event wait, and the settle-point calls the
/// receiver makes.
struct Harness {
...
impl Harness {
    ...
    /// Blocks until the worker tore the pipeline down, so the next file starts
    /// from a settled process.
...
/// Loads one file's media, plays it to EOS and sweeps the sinks. Every panic
/// names `file`, which is the only thing that identifies the case.
fn play_to_eos(handle: &ScenarioHandle, file: &str) {
    ...
    // `harness.audio()` is None when the crate never built an audio sink for
    // this load. An `if let Some(..)` alone would make the audio half of the
    // assertion vanish exactly when it matters, leaving the test green on
    // the video assertions alone.
    assert!(
    ...
    // Only legal at a quiescent point. A snapshot taken mid-flushing-seek
    // ends with an unmatched FLUSH_START and would report a violation.
    assert!(
    ...
    // Deliberately not part of `asserts::all`. The rule only holds for a
    // stream a sink watched open, and nothing here seeks mid-play or loads
    // twice. See fcasttest::sink::asserts::first_buffer_is_discont.
...
/// Plays each file inside `catch_unwind` and collects what failed, so one
/// bad scenario names itself and every later file still runs.
///
...
/// error rather than a hang, so the case is fast and deterministic. They live
/// in `CARGO_TARGET_TMPDIR` rather than `scenario_files/`, because every
/// document in that directory has to pass.
#[test]
fn a_file_that_should_fail_does_and_does_not_hide_the_next_one() {
    ...
    // Named 1 and 2 so the sort order is the order written here. The point
    // is that the first failure does not retire the second file.
    let documents = [
        (
