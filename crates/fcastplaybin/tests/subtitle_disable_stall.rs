...
//!
//! Field report: "disabling subs stalls the video for a bit." No duration in the
//! log, the observation was visual, so this measures it. The video `FTestSink`
//! records a monotonic timestamp per buffer (`RecordEntry::monotonic`), so the
//! stall is exactly the largest wall gap between consecutive rendered buffers
//! inside the disable window, compared against the same statistic over a quiet
