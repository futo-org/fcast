...
//! for video off, the crate's text-without-video guard drops the subtitle out of
//! the selection too, the branch is parked and its disposal POSTPONED (nothing
//! blocking may run on the caller at rest in PAUSED), the `SELECT_STREAMS` goes
//! out with the audio id only, and then nothing happens for 18 s: decodebin3
//! never posts `STREAMS_SELECTED` for that seqnum, so the receiver never
//! confirms the track change.
//!
...
//!    deactivating with no alternative to reassign to, and arms an IDLE probe on
//!    each slot's src pad to run `mq_slot_unassign` (gstdecodebin3.c:4518-4571).
...
//!
//! The crate's video chain is `[subtitleoverlay, video_sink]` with no
...
impl Rig {
    ...
    fn subtitle_pad(&self) -> Option<gst::Pad> {
        self.playbin
            .pipeline()
...
fn disabling_video_while_paused_confirms_the_track_change_on_matroska() {
    ...
    rig.wait_for("the text branch to join the overlay", false, EVENT_TIMEOUT, || {
        rig.subtitle_pad().is_some_and(|pad| pad.is_linked())
    });
