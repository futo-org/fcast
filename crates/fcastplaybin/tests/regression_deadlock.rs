...
//!
//! The wedge is a three-thread cycle: the teardown flush holds the routing
//! lock while waiting for a pad's stream lock, that pad's task holds the
//! stream lock while its routing probe waits for the routing lock, and every
//! further routing-lock caller parks forever.
//!
//! Reproducing the exact cycle would need control over which instant a
//! multiqueue is mid-push, which a test cannot pin down. The invariant behind
//! it can be tested. The teardown flush must not hold the routing lock,
//! because `send_event` runs the entire downstream event chain inline on the
//! calling thread and any blocking anywhere in that chain becomes
//! routing-lock blocking.
//!
...
/// straight to the text queue's sink pad, and that queue feeds the renderer.
///
/// The renderer differs per transport but the flush target does not. The
/// queue sits one element upstream of whichever tail `text_arm` resolves, so
/// this walks back from the tail instead of naming a renderer element.
fn text_branch_flush_target(playbin: &FcastPlaybin) -> gst::Pad {
    let queue_src = text_arm::live_text_tail_pad(playbin)
        .peer()
        .expect("the text branch is linked into the renderer");
    queue_src
        .parent_element()
...
fn the_teardown_flush_does_not_hold_the_routing_lock() {
    ...
    // The flush only reaches a text branch that actually joined the renderer.
    let deadline = Instant::now() + EVENT_TIMEOUT;
    loop {
        if text_arm::text_branch_linked(&playbin) {
            break;
        }
        assert!(
            Instant::now() < deadline,
            "the subtitle branch never reached the renderer"
        );
        playbin.poll_text_policy();
        playbin.pump_selection(gate());
    ...
    // EVENT_FLUSH is not implied by EVENT_DOWNSTREAM. A probe only sees flush
    // events when it asked for that bit, so a downstream-only mask never
    // fires here.
