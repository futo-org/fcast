...
//! engine.
//!
//! Three elements post `GstStreamCollection` onto this bus: urisourcebin
//! (gsturisourcebin.c:3084), decodebin3 (gstdecodebin3.c:2814), and every
//! parsebin inside decodebin3 (gstparsebin.c:3659 and :4219). Only
//! decodebin3's is the MERGED collection, and it is the only one whose stream
//! ids a `SELECT_STREAMS` sent to decodebin3 may name.
//!
//! With a per-stream source every input pad gets its own parsebin, so the
//! single-stream partials arrive interleaved with the merged ones and the
//! collection appears to SHRINK. `SelectionEngine::collection_changed` then
//! reconciles against a collection with no video in it, `resolve` reads the
//! empty video slot as "video off", the no-text-without-video rule strips the
//! subtitle too, and the composed event deselects both. The field symptom was
//! `sent SELECT_STREAMS ids=["...audio_0"]` followed by `selection drops
//! video, parking the video chain at READY` in the middle of a load that
//! asked for neither.
//!
//! decodebin3's own collection grows as its inputs report, which is fine and
//! is what the engine's seeding is built for. It never shrinks mid-load. That
...
fn the_advertised_collection_never_shrinks_during_a_load() {
    ...
    while !playing {
        assert!(Instant::now() < deadline, "the pipeline never settled PLAYING");
        playbin.poll_text_policy();
        playbin.pump_selection(gate());
        while let Ok(event) = events.try_recv() {
            if let PlaybinEvent::Error { error, .. } = &event {
