...
//! scheme to ftestsrc, parsebin plugs ftestparse, decodebin3 autoplugs
//! ftestdec, and the sinks are fcasttest recording sinks handed in through
//! [`Sinks`]. What the harness buys over the real-media suite is control: a
//! buffer parks on a named gate instead of being pushed, so "stop while the
//! source is mid-push" is a fact rather than a race a 7 MB subtitle file makes
//! likely.
...
/// in real time (the sinks sync), so a busy box must not flake: the suite now
/// runs eleven concurrent pipelines, and under a full-machine soak the mid-load
/// dances can trail a plain wait by tens of seconds without anything being
/// wrong.
...
/// The crate's `PREROLL_TIMEOUT` (`src/lib.rs`), mirrored here because it is
/// the ONLY way a start seek is allowed to be skipped on a seekable source:
/// with a healthy preroll `apply_start_seek` either seeks or the source
/// answered the seeking query with "no", and both of those are regressions
/// rather than load. Used to decide whether a start that landed on zero may be
/// forgiven, see [`assert_seeked_start_attach_shares_the_timeline`].
const PREROLL_TIMEOUT: Duration = Duration::from_secs(10);

/// Short clips keep the suite fast. 25 fps and one 20 ms audio packet per
/// buffer, so this is 30 video buffers and 60 audio buffers.
...
/// Source-side pacing for every scenario here. Unpaced: ftestsrc pushes the
/// whole schedule as fast as the chain accepts it and the sinks do the syncing.
///
/// This used to be a 10 ms pre-push delay per buffer. fcasttest caps are
/// sticky, so typefind classifies from the CAPS event alone and forwards the
/// first buffer into the window where parsebin has unlinked `typefind:src` from
/// its parse pad and not yet linked it to the parser. The push returned
/// NotLinked and ftestsrc posted a stream error, so the delay was there to miss
/// the window. ftestsrc now retries a NotLinked push for a bounded while
/// instead (see `NOT_LINKED_BOUND` in `fcasttest::src_bin`), which removes the
/// reason to pace at all.
...
/// A video stream at `fps`, for scenarios whose decoder knobs need a frame
/// budget wider than the 25 fps default.
...
/// Modelled on `tests/subtitle_disable.rs`: same gate, same pumped waits, same
/// log-first matching (events can arrive during preroll, before the wait that
/// needs them).
struct Harness {
    ...
    /// the next settled PAUSED, as the receiver's state machine does. See
    /// `external_subtitle_lifecycle.rs`'s `redrive_transport` for the whole
    /// story; without it a seek refused under load is silently lost.
...
impl Harness {
    ...
    /// Put back a seek the crate parked (its `Job::Seek` refuses a seek that
    /// does not arrive at a settled PAUSED and posts `QueueSeek`; the caller
    /// owns the re-drive). Seek only: this harness's tests drive play/pause
    /// themselves, some deliberately doing work at a loaded PAUSED.
    ...
    /// Load `uri`, start playback, wait for the settled PLAYING. Returns how
    /// long the load itself took (teardown of any previous item included:
    /// the worker resets before it wires the new input).
    ...
    /// The VIDEO anchor pad. The video chain is installed when the item's
    /// video ROUTES, which under load can trail the settled PLAYING the tests
    /// wait on, so this waits bounded instead of expecting instant presence.
    ...
    /// Wait (pumping) until a text branch is wired to its consumer tail.
    /// Panics after `bound`.
    fn wait_subtitle_branch(&self, bound: Duration, what: &str) {
        ...
        while !text_arm::text_branch_linked(&self.playbin) {
            assert!(
                start.elapsed() < bound,
                "the subtitle branch never reached its renderer within {bound:?} ({what})"
            );
            self.settle_pump();
    ...
    /// quiescent point: a snapshot taken mid-flushing-seek ends with an
    /// unmatched FLUSH_START and would report a violation.
...
fn smoke_full_pipeline() {
    ...
    // WAITED for, not read once: decodebin3 may advertise text in a later
    // collection than the one the PLAYING settle rode in on (measured under
    // heavy parallel load), and a single read there is a coin flip.
    let text_sid = {
        let deadline = Instant::now() + EVENT_TIMEOUT;
        loop {
            ...
            assert!(
                Instant::now() < deadline,
                "the collection never advertised the scenario's text stream; log: {:#?}",
                harness.log.borrow()
            );
            std::thread::sleep(Duration::from_millis(10));
        }
...
/// assertion is that text DATA reaches the renderer: 16x16 frames make a
/// pixel-level cue check meaningless, so the real-media suite keeps that job.
#[test]
fn external_subtitle_scenario_reaches_the_renderer() {
    init();
    ...
    // Count cues reaching the renderer, on whichever transport this process
    // drives, from BEFORE the attach. Not after: the consumer branch is
    // unsynced, so this input's whole schedule is handed over within
    // milliseconds of the branch linking -- measured here at ~2ms after the
    // attach, which is inside the materialization poll the test would
    // otherwise take its baseline behind.
    let text_seen = text_arm::count_text_arrivals(&harness.playbin);
    let text_before = text_seen.count();
    ...
    let deadline = Instant::now() + EVENT_TIMEOUT;
    loop {
        if text_seen.count() > text_before {
            break;
        }
        assert!(
            Instant::now() < deadline,
            "no cue reached the renderer; log: {:#?}",
            harness.log.borrow()
        );
        harness.settle_pump();
        std::thread::sleep(Duration::from_millis(10));
...
fn assert_seeked_start_attach_shares_the_timeline(
    ...
    // Sticky segments only exist once data flowed: wait for the branch's tail
    // to carry one.
    let deadline = Instant::now() + EVENT_TIMEOUT;
    while text_arm::live_text_origin(&harness.playbin).is_none() {
        assert!(
            Instant::now() < deadline,
            "no text segment reached the renderer; log: {:#?}",
            harness.log.borrow()
        );
        harness.settle_pump();
        std::thread::sleep(Duration::from_millis(10));
    ...
    loop {
        ...
        let text_origin = text_arm::live_text_origin(&harness.playbin);
        ...
        if Instant::now() >= deadline {
            ...
            assert!(
                ...
                "the item's VIDEO never reached the sought timeline: the origin at \
                 the video sink is {video_origin:?}, expected \
                 ...
                 segment that reaches the sink is the pre-seek one. The seek is \
                 ...
                 `video_timeline` (external subtitles above all) is then shifted \
...
fn tap_overlay_text(harness: &Harness) -> TextTap {
    text_arm::tap_cue_positions(&harness.playbin)
}
...
/// The cues of `prefix` this tap could PLACE on a timeline.
///
/// AN UNREADABLE ORIGIN IS NOT A MISALIGNED CUE. `PositionTap` reads the origin
/// off the sticky SEGMENT on the consumer appsink's sink pad, and cues handed
/// over out of the text park (`Inner::take_parked_text_cues`) reach the
/// consumer DIRECTLY, before the rebuilt branch's first push has stickied
/// anything there. They carry their own segment in the `gst::Sample`, so they
/// are placed correctly and only the TAP cannot see it. On an
/// external-to-external switch that is the whole opening of the switched-to
...
fn placed_with_prefix(tap: &TextTap, prefix: &str) -> Vec<(String, gst::ClockTime)> {
    tapped_with_prefix(tap, prefix)
        .into_iter()
        .filter_map(|(payload, origin)| origin.map(|origin| (payload, origin)))
        .collect()
}

/// [`wait_for_prefixed_cue`] for a cue whose origin is READABLE. Panics on
/// timeout, so "every cue was unreadable" is a failure and never a vacuous
/// pass.
fn wait_for_placed_cue(
    harness: &Harness,
    tap: &TextTap,
    prefix: &str,
    what: &str,
    ...
    let deadline = Instant::now() + EVENT_TIMEOUT;
    loop {
        let seen = placed_with_prefix(tap, prefix);
        if !seen.is_empty() {
            return seen;
        }
        assert!(
            Instant::now() < deadline,
            "no placeable {prefix} cue reached the renderer ({what}); log: {:#?}",
            harness.log.borrow()
        );
        harness.settle_pump();
        std::thread::sleep(Duration::from_millis(10));
...
fn wait_for_prefixed_cue(
    ...
    loop {
        ...
        assert!(
            Instant::now() < deadline,
            "no {prefix} cue reached the renderer ({what}); log: {:#?}",
            harness.log.borrow()
        );
        harness.settle_pump();
        std::thread::sleep(Duration::from_millis(10));
...
fn switching_external_subtitles_realigns_the_new_input() {
    ...
    let sub_a = ScenarioBuilder::new("swrealigna")
        .text(
            "text_0",
            prefixed_cues("AAA", 30, gst::ClockTime::from_mseconds(400)),
        )
        .duration(gst::ClockTime::from_seconds(15))
        .pacing(PACING)
        .register();
    let sub_b = ScenarioBuilder::new("swrealignb")
        .text(
            "text_0",
            prefixed_cues("BBB", 30, gst::ClockTime::from_mseconds(400)),
        )
        .duration(gst::ClockTime::from_seconds(15))
        .pacing(PACING)
        .register();
    ...
    while segment_origin(&video_pad) != Some(origin) {
        // DRAINED, not merely pumped: a seek refused under load comes back as
        // `QueueSeek`, and only event processing re-drives it
        // (`redrive_transport`). This loop left it unread in the channel
        // forever, with the pad's pristine pre-seek segment as the proof.
        harness.drain_events();
        assert!(
            Instant::now() < deadline,
            "the video origin never moved to {origin}; video pad sticky segment: {:?}; log: {:#?}",
    ...
    for (payload, cue_origin) in wait_for_placed_cue(&harness, &tap, "AAA", "baseline") {
        assert_eq!(
            cue_origin, origin,
    ...
    let fresh = wait_for_placed_cue(&harness, &tap, "BBB", "after the switch");
    for (payload, cue_origin) in fresh.iter().chain(placed_with_prefix(&tap, "BBB").iter()) {
        assert_eq!(
            *cue_origin, origin,
...
fn reactivating_an_external_subtitle_replays_it() {
    ...
    let sub_a = ScenarioBuilder::new("reacta")
        .text(
            "text_0",
            prefixed_cues("AAA", 20, gst::ClockTime::from_mseconds(400)),
        )
        .duration(gst::ClockTime::from_seconds(30))
        .pacing(PACING)
        .register();
    let sub_b = ScenarioBuilder::new("reactb")
        .text(
            "text_0",
            prefixed_cues("BBB", 20, gst::ClockTime::from_mseconds(400)),
        )
        .duration(gst::ClockTime::from_seconds(30))
        .pacing(PACING)
        .register();
...
fn reactivating_a_dead_external_subtitle_replays_it() {
    ...
    let sub_a = ScenarioBuilder::new("deadreacta")
        .text(
            "text_0",
            prefixed_cues("AAA", 90, gst::ClockTime::from_mseconds(250)),
        )
        .duration(gst::ClockTime::from_seconds(25))
    ...
    let sub_b = ScenarioBuilder::new("deadreactb")
        .text(
            "text_0",
            prefixed_cues("BBB", 90, gst::ClockTime::from_mseconds(250)),
        )
        .duration(gst::ClockTime::from_seconds(25))
        .pacing(PACING)
        .register();
    ...
    select_and_confirm(&harness, id_a, &sid_a);
    wait_for_prefixed_cue(
        &harness,
        &tap,
        "AAA",
        seen_before,
        "reactivating the dead input",
    );
...
fn dropping_the_playbin_with_a_held_external_is_bounded() {
    ...
    let subs = ScenarioBuilder::new("dropheldsubs")
        .text(
            "text_0",
            prefixed_cues("HLD", 10, gst::ClockTime::from_mseconds(400)),
        )
        .duration(gst::ClockTime::from_seconds(10))
        .pacing(PACING)
        .register();
...
fn external_srt_attached_during_seeked_start_shares_the_timeline() {
    ...
    let path =
        std::env::temp_dir().join(format!("fcastplaybin-seekstart-{}.srt", std::process::id()));
...
/// the branch's own tail, which no upstream event travelling up
...
fn seeked_start_puts_the_items_own_text_on_the_sought_timeline() {
    ...
    let main = ScenarioBuilder::new("seekstartinternal")
        ...
        // a text tail with no segment at all and this test measuring nothing.
    ...
    loop {
        ...
        assert!(
            ...
            "the seeked start left the item's own streams on different timelines: \
             the origin at the video sink is {video_origin:?} and at \
             the text branch's tail {text_origin:?}, both expected {start:?}. A start \
             seek that only reaches the SINK-flagged children of the pipeline never \
             gets to the text branch at all. log: {:#?}",
            harness.log.borrow()
        );
        harness.settle_pump();
        std::thread::sleep(Duration::from_millis(10));
