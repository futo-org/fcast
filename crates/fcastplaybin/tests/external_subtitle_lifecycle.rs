...
impl Harness {
    ...
    /// The overlay element, waited for: the video chain (overlay included)
    /// is installed when the item's video ROUTES, which can trail the
    /// settled PLAYING.
    ...
    fn overlay_video_pad(&self) -> gst::Pad {
        self.overlay()
            .static_pad("video_sink")
            .expect("subtitleoverlay has a video_sink pad")
    }

    /// What is wired into the overlay's subtitle input right now, walked
    /// back to the decodebin3 pad feeding it. The link policy refuses to
    /// join a text stream while this pad is occupied, so when a subtitle
    /// stops rendering this says WHICH branch is holding the seat.
    fn subtitle_sink_occupant(&self) -> String {
...
fn a_second_external_renders_after_the_selected_one_was_detached() {
    ...
    // The seat in subtitleoverlay must be free again. A stale branch left
    // linked there makes the link policy skip EVERY later text stream
    // ("subtitle_sink already linked"), which kills subtitles for the rest
    // of the item without any error being reported.
...
fn detaching_an_unselected_external_while_paused_returns() {
    ...
    assert!(
        ...
        "the control needs an UNLINKED text branch, but one reached the \
         overlay anyway: {}",
        harness.subtitle_sink_occupant()
    );
    harness.pause();
...
fn an_external_carrying_av_as_well_as_text_renders_only_its_text() {
    ...
    assert!(
        ...
        "the item's video stopped once a video-bearing external was attached"
    );
...
fn attach_then_seek_keeps_the_external_on_the_video_timeline() {
    ...
    let (_id, _sid) = harness.attach_and_select(&subs.uri());
    let before = harness.wait_for_cue(&tap, "ATSK", 0, "before the seek");
    assert_origins(&before, gst::ClockTime::ZERO, "before the seek");
    let already = before.len();
...
fn attaching_during_a_seek_still_lands_on_the_sought_timeline() {
    ...
    let video_pad = harness.overlay_video_pad();
    harness.wait_until("the video origin to move", EVENT_TIMEOUT, || {
        segment_origin(&video_pad) == Some(origin)
    });
    harness.play();
...
fn seek_to(harness: &Harness, position: gst::ClockTime) {
    ...
    let video_pad = harness.overlay_video_pad();
    harness.wait_until(
        &format!("the video origin to reach {position}"),
        EVENT_TIMEOUT,
        || segment_origin(&video_pad) == Some(position),
    );
    harness.play();
}
