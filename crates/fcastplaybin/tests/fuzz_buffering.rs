...
impl Case {
    fn generate(seed: u64, iteration: u64, actions: usize) -> Self {
        ...
        let pair_reaches_pool = schedule
            .iter()
            .rev()
            .take_while(|(_, action)| !matches!(action, Action::StopReload | Action::LoadSwap));
        let has_select = pair_reaches_pool
            .clone()
            .any(|(_, action)| matches!(action, Action::SelectExternal(_)));
...
fn generate_media(
    ...
    match prng.next_range(0..4) {
        0 | 1 => {
            buffering =
                buffering.with_periodic(prng.next_range(900..2200), prng.next_range(120..650));
        }
        2 => {
        ...
        match anchor {
            ...
            Anchor::Buffers(count) => {
                ...
                while self.video.buffer_count() < target && Instant::now() < deadline {
                    let _ = self.pump();
                    self.video
                        .wait_for_buffers(target, Duration::from_millis(20));
                }
            }
            Anchor::Settled => {
