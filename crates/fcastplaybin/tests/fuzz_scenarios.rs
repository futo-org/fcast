//! Seeded stress driver: random media, random action schedules, invariants
//! after every action. Opt-in, so a normal `cargo test` never pays for it.
//!
//! ```text
...
//! action order, the slot and seek position every action carries, and the
//! anchor each action waits on. No `thread_rng`, no clock reads, so
...
//! first, take the embedded text, drop video, seek, take video back, seek
//! again, attach a third without selecting it, detach, select the newest, seek,
//! pause, resume. The schedule is a random walk over exactly those moves, so
//! every interleaving of them is reachable from some seed.
//!
...
//!
//! A failing iteration is written to `target/fuzz-failures/` as the scenario
...
/// selection can name it. Soft: an input that has not materialized yet is
/// simply not selected this step, which is a lost permutation and not a
/// failure.
...
/// Longest an anchor waits before the action just proceeds. An anchor is a
/// bias, not a correctness wait: missing it is not a failure.
...
/// nothing has crossed the overlay yet. Much shorter than [`ALIGN_BOUND`]
/// (which exists for a misalignment to converge) purely for cost: this look
/// happens on every selection sweep, including the many that legitimately have
/// no cue due yet, and burning the full bound on each would multiply the
/// driver's runtime.
...
/// Payload prefix per external slot, so a dump names which input a cue came
/// from.
...
/// What an action waits for before it runs. Uniform sleeps find nothing: the
/// bugs this replaces all lived at a lifecycle edge.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Anchor {
...
/// yet) and only biases the slot draws toward slots the runner will plausibly
/// be able to act on: a schedule that keeps selecting slots it never attached
/// is a schedule of no-ops, and the permutations under test all need a
/// populated pool. The model can be wrong at run time (the crate may refuse an
/// attach), which costs coverage and never correctness.
...
/// Media with a video and an audio stream, sometimes an embedded text stream,
/// and sometimes a stall gate on the video. No error or EOS fault is ever
/// injected, so "the pipeline posted an error" stays an unambiguous failure.
/// Returns the duration too: the seek positions in the schedule are drawn
/// against it.
fn generate_media(
    prng: &mut Prng,
    key: &str,
...
/// A failure carries everything needed to write a replayable case. Checks
/// return this instead of panicking so the driver can dump and, optionally,
/// shrink.
struct Failure {
...
struct Runner {
    ...
    /// Loads issued so far. The caller-owned video log only belongs to one
    /// load, so the sweep drops it once a second load happened (a swap's
    /// teardown legally leaves the log ending mid-flush).
    loads: Cell<u32>,
    ...
    /// One entry per cue that reached the renderer: the video's timeline
    /// origin at that instant and the cue's own. See [`Runner::tap_text_tail`].
    ...
    /// The text tail pads the tap is installed on, so a rebuilt branch is
    /// tapped again and a surviving one is not tapped twice. SHARED with the
    /// `deep-element-added` hook, which runs on whichever thread builds the
    /// branch, so it is a Mutex rather than a RefCell.
    tapped: Arc<Mutex<Vec<gst::Pad>>>,
    /// Whether the `deep-element-added` hook is armed (once per pipeline; the
    /// pipeline outlives every load).
    tap_hooked: Cell<bool>,
}
...
impl Runner {
    fn new() -> Self {
        ...
        let runner = Self {
            ...
            tapped: Arc::new(Mutex::new(Vec::new())),
            tap_hooked: Cell::new(false),
        };
        // ARMED HERE, before a single load. The hook is what catches a branch
        // as it JOINS (see `tap_text_tail`), and each iteration's load builds
        // its text branch during `load_and_play` -- so a hook connected at the
        // END of that call has already missed it. Measured that way round:
        // nine branch joins over a run, two sweeps with any data in them.
        runner.tap_text_tail();
        runner
    }
    ...
    /// injects an error, so any error at all is one. `ExternalSubtitleFailed`
    /// is the same kind of statement about the externals: every generated
    /// URI is valid and resolves to a registered scenario, so the crate
    /// reporting one dead means it gave up on a live input. A death handled
    /// by the recover path never reaches this event, so it is not noisy.
    ...
    /// Moves every pending event into the log. Returns whether `pred` matched
    /// one of them, plus the first problem it saw.
    fn drain_matching(
        &self,
    ...
    /// Moves every pending event into the log. Returns the first problem it
    /// saw.
    ...
    /// Sinks whose log provably belongs to the current load, see
    /// [`Runner::loads`].
    ...
    /// First stream of `kind` in the latest collection that no attached
    /// external input owns, so this always names one of the play item's own
    /// streams.
    ...
    /// One BUFFER probe on a consumer tail's sink pad, recording the pair of
    /// running-time ORIGINS each cue crosses on: the video's and its own.
    /// Skips a pad already in `tapped`, and does nothing for an element that
    /// is not a text tail. See [`Runner::tap_text_tail`].
    fn probe_text_tail(
        element: &gst::Element,
        video: &gst::Element,
        record: &Arc<Mutex<Vec<(gst::ClockTime, gst::ClockTime)>>>,
        tapped: &Arc<Mutex<Vec<gst::Pad>>>,
    ) {
        if !element.name().starts_with("fpb-textsink-") {
            return;
        }
        let Some(text) = element.static_pad("sink") else {
            return;
        };
        {
            let mut held = tapped.lock().expect("alignment tap set");
        ...
        text.add_probe(gst::PadProbeType::BUFFER, move |pad, _| {
            ...
            let video_origin = video
                .static_pad("sink")
                .and_then(|pad| segment_origin(&pad));
            if let (Some(text_origin), Some(video_origin)) = (segment_origin(pad), video_origin) {
                record
                    .lock()
    ...
    /// Tap the live text branch's TAIL so the timeline invariant judges DATA
    ...
    ///
    /// # It has to be ARMED before the branch links, not polled for
    ///
    /// Before subtitleoverlay was deleted there was ONE tail for the whole run,
    /// subtitleoverlay's static `subtitle_sink`, so a tap taken at any time
    /// caught every cue. A branch builds its OWN appsink now and takes it away
    /// again, and the branch is UNSYNCED: an external subtitle is parsed and
    /// handed over in full within milliseconds of linking. A tap that waits
    /// for the element to appear and then probes it arrives after the burst,
    /// and the record stays empty for a branch that delivered everything it
    /// had. The same hazard `support/text_arm.rs`'s `arm()` exists for.
    ///
    /// So the hook goes on `deep-element-added` and probes each appsink as it
    /// JOINS, before it can carry a buffer; the sweep afterwards covers a
    /// branch that beat the hook. Idempotent per pad.
    ///
    /// ARMED IN [`Runner::new`], which is not a detail: each iteration's load
    /// builds its text branch inside `load_and_play`, so a hook connected when
    /// that call RETURNS has already missed it, and only the branches a later
    /// action builds are ever seen.
    fn tap_text_tail(&self) {
        if !self.tap_hooked.replace(true) {
            let record = self.alignment.clone();
            let tapped = self.tapped.clone();
            // The video sink ELEMENT, not its pad: the element lives as long
            // as the pipeline, while its pad's membership comes and goes with
            // `ensure_video_chain`/`remove_video_chain`.
            let video = self.playbin.video_sink();
            self.playbin
                .pipeline()
                .connect_deep_element_added(move |_, _, element| {
                    Runner::probe_text_tail(element, &video, &record, &tapped);
                });
        }
        let video = self.playbin.video_sink();
        for pad in text_arm::text_tail_pads(&self.playbin) {
            if let Some(element) = pad.parent_element() {
                Runner::probe_text_tail(&element, &video, &self.alignment, &self.tapped);
            }
        }
    }
    ...
    /// walks every cue recorded since the last passing check
    /// (`alignment_cursor`) and forgives at most [`FORGIVEN_STRAGGLERS`] in
    /// a row.
    ///
    ...
    /// Bounded poll for a settled pipeline state. Reaching it is a bias and not
    /// a correctness property (the transport is allowed to refuse), so the
    /// bound expiring proceeds rather than fails. It no longer proceeds
    /// SILENTLY: the bool says whether the state was actually reached, and
    /// the caller must not book unperformed work as coverage. A problem
    /// event is still a failure.
    ...
    fn load_and_play(&self, uri: &str) -> Checked {
        ...
        self.tapped.lock().expect("alignment tap set").clear();
        ...
        // The sweep half, for a branch that somehow beat the hook armed in
        // `Runner::new`. The hook is what does the work.
        self.tap_text_tail();
        Ok(())
    }
    ...
    /// Bias only. Reaching the anchor is not required, so a missed one is
    /// silent.
    ...
    /// The stream id an attached slot produced, once it materialized. Bounded
    /// and soft: an input still coming up is simply not selected this step.
    ...
    /// The receiver's mid-stream seek. A seek issued straight from PLAYING
    /// parks waiting for the caller's state machine to re-drive it, so the
    /// dance is the only shape that actually moves the timeline (see
    /// `switching_external_subtitles_realigns_the_new_input` in
    /// `tests/scenarios.rs`). Every intermediate wait is soft, the two
    /// transport calls are not.
    ...
    /// Externals belong to the play item and die with the load's input reset,
    /// so every load starts from an empty pool.
...
/// Runs one case start to finish. Every action is followed by the invariant
/// set. A pass returns the case's [`Coverage`]; a failure is the failure.
...
/// Writes the case as scenario files plus the action list. Returns the
/// directory, or the reason it could not be written (never a failure of the run
/// itself).
...
/// Greedy drop-one shrink. Timing changes when an action goes, so a candidate
/// that stops failing proves nothing and is simply kept out of the result.
...
/// The whole driver. Ignored by default: it runs real pipelines for as long as
/// the iteration count says.
#[test]
