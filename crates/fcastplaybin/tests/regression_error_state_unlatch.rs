...
//! errors deliberately, but the latch stays, so a flushing seek's lost state
//! is never committed. Sinks preroll, ASYNC_DONE arrives, the commit is
//! refused, nothing is posted, and the caller's `StateMachine` sits on an
//! in-flight seek forever. Determinism comes from releasing the error inside
//! the lost-state window, held open by parking the main video on a sync point
//! the seek's restart runs into.
...
/// fps). `start_index` takes the first item ending past the offset, and with
/// a keyframe interval of 1 it does not walk back.
...
fn main_item(key: &str) -> ScenarioHandle {
    let video = StreamSpec::new(
        ...
        StreamKind::Video {
            ...
            // SEEK_TARGET_FRAME itself, the frame the stall is on.
            keyframe_interval: 1,
        },
    )
...
/// posts the exact flow-error text the crate classifies by, and only
/// "not-linked" and "flushing" are `Recover`.
///
/// Do NOT use `Fault::ErrorAt`. It classifies as `Fail`, which is
/// self-healing here (the detach's own `set_state` clears the latch), so the
/// test would pass in both arms and prove nothing.
fn sub_item(key: &str, gate: &str) -> ScenarioHandle {
    ...
    let text = StreamSpec::text("text_0", cues)
        ...
        // and it survives the seek, since an unselected external is never
        // seeked.
        .with_fault(Fault::StallAt {
            buffer_index: 0,
...
/// drives it. A hand-rolled waiter would not do. Only a real [`StateMachine`]
/// can tell "the settle was announced" from "the pipeline happens to be at
/// PAUSED".
struct Harness {
    ...
    fn pump(&self) {
        ...
        while let Ok(event) = self.events.try_recv() {
            match event {
                ...
                PlaybinEvent::Error { error, origin, .. } => self.note(format!(
                    "the pipeline posted an error: {error} ({origin:?})"
                )),
                // The external's death must be consumed. If it is reported,
                // the injected error classified `Fail` and this is no longer
                // a control (see `sub_item`).
...
/// Pins the coupling this file rests on. fcasttest's injected text must be
/// the text the crate recovers on. Unit tests pin the other end, so drift is
/// a failing test rather than a regression test that quietly stops
/// discriminating.
#[test]
...
fn drive_into_the_lost_state_window(
    ...
    // assumed, because auto-select can route a fresh external before anyone
    // asked and the crate corrects that asynchronously.
    harness
        .playbin
    ...
    for id in ids {
        harness.wait_until(
            "every external to expose its text stream",
            BOUND,
            move |h| !h.playbin.subtitle_stream_ids(id).is_empty(),
        );
    }
    ...
    // Each attach puts an ASYNC_START/ASYNC_DONE pair on the bus. Seeking
    // with one in flight lets the machine consume its settled-Paused edge as
    // the seek's settle, so the slot clears before the flush goes out and the
    ...
    // Arrivals rather than `waiting`, since a parked task registers the
    // arrival once and `waiting()` reads zero throughout. Counted from before
    ...
    // Both halves are needed. The parked restart proves the flush went out
    // and the sink cannot re-preroll, and `(Paused, Paused)` is the lost
    // state itself. The park alone would also match the earlier
    // PLAYING->PAUSED dip performed before the real seek.
...
fn a_consumed_error_still_lets_a_seek_settle() {
    ...
    // Hold the window open until the consumed error has been seen. With the
    // fix, the re-commit reports the settle and this returns early. With the
    // lever, nothing is reported and it runs its bound, which puts the error
    // inside the window rather than after the commit.
    ...
    // Diagnostic only. Records whether the settle was announced promptly or
    // arrived later by some other route.
...
/// pipeline reads `(Paused, Playing)`. The second error latches FAILURE on
/// exactly that. Re-committing the current state (PAUSED) there retargets the
/// pipeline back to PAUSED and announces nothing.
#[test]
fn a_consumed_error_on_a_pending_climb_keeps_the_climb() {
    ...
    // Nothing to wait for, since the second error's whole effect is on the
    // pending state, which nothing announces. A bounded pump gives it time to
    // land before the sink is allowed to preroll.
...
/// Scrubbing during the lost-state window must not lose the seek.
///
/// Two consumed errors run their unlatch jobs back-to-back, each emitting a
/// settle report while the pipeline is still async. The machine takes the
...
fn scrubbing_inside_the_lost_state_window_keeps_the_seek() {
    ...
    let before = drive_into_the_lost_state_window(
        &harness,
        &main,
        &[(&first, EXT_GATE), (&second, EXT_GATE)],
    );
    ...
    // window and are still tracked when it closes. The window is held by
    // MAIN_HOLD, which the fix never touches, so these hold in both arms or
    // the rig itself broke.
    assert_eq!(
        harness.playbin.state_summary(),
    ...
    // held pipeline deterministically. Then the window closes and the
    // receiver must converge on its own.
    first.release_all();
    second.release_all();
    let _ = harness.pump_until(Duration::from_millis(400), |_| false);
    main.release(MAIN_HOLD);
