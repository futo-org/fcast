...
//! `Inner::flush_pads` (lib.rs:5848) sends FlushStart + FlushStop(false) on
...
fn watch_warnings() -> (Arc<Mutex<Vec<String>>>, Arc<AtomicBool>) {
    ...
    gst::glib::log_set_default_handler(move |domain, level, message| {
        // Re-print, so a failing run stays readable, EXCEPT the two families
        // this suite exists to manufacture ("before segment event" and the
        // segment-format critical it cascades into). Those are the measured
        // phenomenon, recorded below for the cross-check either way; printing
        // them streams hundreds of lines past libtest's capture (this handler
        // runs on streaming threads) on every PASSING run, which buries any
        // real warning. `FCAST_CENSUS_LOUD=1` restores the full stream for a
        // failing investigation.
        let manufactured = message.contains("before segment event")
            || message.contains("segment->format == format");
        if !manufactured || std::env::var_os("FCAST_CENSUS_LOUD").is_some() {
...
fn install_census(playbin: &FcastPlaybin, census: &Census) -> Vec<gst::Pad> {
            ...
            move |pad, info| {
                let mark = match &info.data {
                    Some(gst::PadProbeData::Buffer(_)) | Some(gst::PadProbeData::BufferList(_)) => {
                        Mark::Buffer {
                            had_segment: pad.sticky_event::<gst::event::Segment>(0).is_some(),
                        }
                    }
    ...
    assert!(
        !watched.is_empty(),
        "fpb-decodebin had no sink pads to watch"
    );
    watched
}
...
/// the live text branches' tails, and returns the pads it watched. Empty is
/// a legitimate answer (no text branch has joined yet), so this asserts
/// nothing by itself - callers decide.
///
/// `EVENT_FLUSH` must be requested EXPLICITLY here for the same reason as in
/// [`install_census`]: `EVENT_DOWNSTREAM` does not imply it, and a flush
/// census that silently drops flushes is worse than none.
fn install_text_census(playbin: &FcastPlaybin, census: &Census) -> Vec<gst::Pad> {
    let pipeline = playbin.pipeline();
    let mut watched: Vec<(String, gst::Pad)> = Vec::new();
    for element in pipeline.iterate_elements().into_iter().flatten() {
        let name = element.name().to_string();
        if !name.starts_with("fpb-tqueue-") {
            continue;
        }
        if let Some(pad) = element.static_pad("sink") {
            watched.push((format!("{name}:sink"), pad));
        }
    }
    // The branch TAIL: the per-stream appsinks' sink pads on the
    // consumer arm. Same event stream, same question, one element along.
    for pad in text_arm::text_tail_pads(playbin) {
        watched.push((text_arm::text_tail_pad_name(&pad), pad));
    }
    let mut pads = Vec::new();
    for (name, pad) in watched {
        let sink = census.clone();
        pad.add_probe(
            gst::PadProbeType::BUFFER
                | gst::PadProbeType::BUFFER_LIST
        ...
        .expect("text census probe");
        pads.push(pad);
    }
    pads
}

/// FLUSH_START marks per pad, for the report-only text census.
fn flush_starts(census: &Census) -> Vec<(String, usize)> {
    let log = census.lock().expect("census");
    let mut per_pad: Vec<(String, usize)> = Vec::new();
    for (pad, mark) in log.iter() {
        if *mark == Mark::FlushStart {
...
fn an_injected_flush_pair_strands_a_decodebin_sink_pad_without_a_segment() {
    ...
    // Byte-for-byte `Inner::send_flush_pair`. The `reset_time = false` is
    // the crate's (see there); the sticky removal this test is about is
    // unconditional either way (gstpad.c:5919).
    for pad in &targets {
...
fn stopping_flushes_the_decodebin_sink_pads_while_the_sources_still_push() {
    ...
    let census: Census = Arc::new(Mutex::new(Vec::new()));
    install_census(&rig.playbin, &census);
    // The text branch too, report-only: this item carries no text, so the
    // watched set is normally the live branch's tail alone (or empty,
    // before any branch has been built). It is here
    // so a stop that starts flushing text pads shows up.
    let text_census: Census = Arc::new(Mutex::new(Vec::new()));
    let text_pads = install_text_census(&rig.playbin, &text_census);
    {
        let census = census.clone();
        rig.wait_until("buffers to flow into decodebin3", move || {
            census
    ...
    eprintln!(
        "STOP text census ({} pads watched): FLUSH_START per pad {:?}\n{}",
        text_pads.len(),
        flush_starts(&text_census),
        render(&text_census)
    );
    rig.main.unregister();
}
...
fn detaching_an_external_flushes_its_decodebin_pad_without_a_replacement_segment() {
    ...
    // THE detach is what puts pairs on the text branch (`detach_text_parts` ->
    // `dispose_text_branch`, plus `remove_input`'s own pair travelling down),
    // so this is the schedule the text census exists for.
    let text_census: Census = Arc::new(Mutex::new(Vec::new()));
    let text_pads = install_text_census(&rig.playbin, &text_census);
    eprintln!("watching {} text-branch pads", text_pads.len());
    rig.observe(Duration::from_millis(400));
    ...
    eprintln!(
        "DETACH text census: FLUSH_START per pad {:?}\n{}",
        flush_starts(&text_census),
        render(&text_census)
    );
