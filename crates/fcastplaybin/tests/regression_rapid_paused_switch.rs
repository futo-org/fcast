...
//!
//! 1. `verify_replay` (lib.rs, "Worker side of the replay verification")
//!    reads the branch's sticky STREAM_START and sticky segment to decide
//!    "aligned delivery, no replay needed". While the pipeline is held below
//!    PLAYING nothing flows, so those stickies are leftovers of the input's
//!    PREVIOUS tenure, and a spent input passes the check. Captured as the
//!    final arming firing its check about 400 ms later, still parked, and
//!    returning silently through the aligned path. Checks that fire mid
//!    alternation instead log "selection moved on; not replaying" and also
//!    conclude nothing for the final selection.
//! 2. Nothing re-verifies after the resume. The arming is edge-triggered by
//!    the STREAMS_SELECTED confirmation, and that edge is either consumed
//!    while parked (feeding defect 1) or never posted at all when the
//!    backlog stalls decodebin3, in which case no arming ever exists for
//!    the final selection. The deferred-work drain re-attempts flushes,
//!    disposals, owed replays and input removals but never re-checks
//!    whether the settled selection is actually delivering. Both captured
//!    shapes end identically.
//!
...
fn rapid_external_switches_while_parked_in_buffering_recover_on_completion() {
    ...
    // Text crossing into the overlay, observed with its payload so the
    // recovery can be pinned to the LAST selected track rather than to any
    // stale backlog. The overlay joins the pipeline with the video chain, so
    // its appearance is waited out rather than assumed.
    receiver.wait_until("the subtitle overlay to join the pipeline", false, |r| {
        r.playbin.pipeline().by_name("fpb-suboverlay").is_some()
    });
    let overlay_subtitle = receiver
        .playbin
        .pipeline()
            ...
            .add_probe(gst::PadProbeType::BUFFER, move |_, info| {
                    ...
                    && let Ok(map) = buffer.map_readable()
                {
                    texts
                        .lock()
                        .expect("texts")
                        .push(String::from_utf8_lossy(&map).into_owned());
                }
                gst::PadProbeReturn::Ok
            })
            .expect("recording text into the overlay");
    }
        ...
        loop {
            receiver.pump(false);
            {
                ...
                if texts[texts_before_release..]
                    .iter()
                    .any(|text| text.starts_with('B'))
                {
                    break;
                }
                if Instant::now() >= deadline {
                    let after: Vec<&String> = texts[texts_before_release..].iter().collect();
                    ...
                    panic!(
                        "the last selected track's text never reached the overlay within \
                         ...
                         subtitle_sink linked={}, post-release text buffers={} tail={tail:?}). \
