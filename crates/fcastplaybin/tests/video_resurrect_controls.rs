...
//! own binary because the red test there ends in `std::process::exit`,
//! which would take sibling tests with it.
//!
...
//! unconditionally self-heals.
//!
//! The harness is a duplicate of the red test's, kept in sync by hand. See
//! the module comment there for the full mechanism.
...
/// unconditionally self-heals. The re-assert dispatches mid transition,
/// decodebin3 drops the resurrected video pad, and the re-preroll unblocks.
#[test]
