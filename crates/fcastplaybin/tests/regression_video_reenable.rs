...
fn media(key: &str) -> ScenarioHandle {
    ...
    //   throttles a DESELECTED stream, so a short clip hits EOS within ms and
    //   the drained-resurrect park rightly refuses the re-route: fails in BOTH
    //   arms.
    //
...
fn video_off_then_on_mid_item_keeps_rendering() {
    ...
    harness.pump();
    harness.wait_until("the video re-enable to confirm", SETTLE_BOUND, |h| {
        matches!(&*h.selected_video.borrow(), Some(Some(got)) if *got == sid)
    });
    harness.wait_until("the pipeline to settle after the re-enable", SETTLE_BOUND, |h| {
        h.playbin.state_summary() == (gst::State::Playing, gst::State::VoidPending)
            && !h.playbin.has_async_transition()
    });
    // Settled is not enough: a joined chain that gets no data reads settled too.
    harness.wait_until("video to render again after the re-enable", SETTLE_BOUND, |h| {
        h.rendered() > before
    });
