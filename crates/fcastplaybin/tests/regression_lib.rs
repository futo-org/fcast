...
fn wait_for_seek(log: &SeekLog, seqnum: gst::Seqnum, what: &str) -> SeenSeek {
    ...
    loop {
        if let Some(seen) = log
            .lock()
            .unwrap()
            .iter()
            .rev()
            .find(|s| s.seqnum == seqnum)
        {
            return *seen;
        }
        assert!(
...
/// `Seek { position: None }` shape a SetSpeed produces: the caller owns the
/// seek queue and marks the seek in flight, so a job that neither seeks nor
/// reports leaves that slot in flight with nothing left to settle it, and every
/// later seek parks behind it.
///
