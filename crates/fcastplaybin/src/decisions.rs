...
/// are unknowable, so never deactivate then.
...
/// only when the format is both implemented
/// ([`super::bitmap_format_implemented`]) and enabled (the
/// `FCAST_NO_BITMAP_SUBS` family). Anything else gets `None`, which keeps
/// the master lever a bit-for-bit restoration rather than a second code
/// path.
///
...
/// arm and the `FCAST_NO_CUE_IR` lever changes nothing in it. The variant
...
/// `text/x-raw`.
pub(crate) fn consumer_stream_format(
    caps: &gst::CapsRef,
    enabled: crate::levers::BitmapSubsEnabled,
) -> Option<ConsumerStreamFormat> {
    ...
    (enabled.allows(format) && super::bitmap_format_implemented(format))
        .then_some(ConsumerStreamFormat::Bitmap(format))
}
