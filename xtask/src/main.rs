...
enum Command {
    ...
    /// Default lane for the receiver side. The sender crates live in
    /// `test-sender`.
    Test,
    /// Quick lane of pure element tests. They build a factory and drive it,
    /// so they skip the whole UI dependency tree.
    TestQuick,
...
fn main() {
    match Xtask::parse().cmd {
        ...
        Command::TestQuick => {
            ...
            println!(">> quick lane: element tests (no slint)");
