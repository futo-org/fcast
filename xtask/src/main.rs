...
fn main() {
    match Xtask::parse().cmd {
        ...
        Command::Test => {
            ...
            cmd!(sh, "cargo test --all-targets --all-features --workspace --exclude receiver-android --exclude android-sender --exclude fiatlux-sys --exclude fiatlux --exclude fhs-receiver --exclude xtask-fuzz --exclude libplacebo-sys --exclude receiver-resources --exclude get-type-string-derive --exclude egl-sys").run().unwrap();
        }
